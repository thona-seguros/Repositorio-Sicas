PROCEDURE NOTIFICACOBRANZADCL2 (nCodCia NUMBER, nCodEmpresa NUMBER, nIdPoliza NUMBER, nIDetPol NUMBER, nIdFactura NUMBER) IS
...
--                                  AND RE.IdProceso  = RE.IdProceso)
    ...
    cMessage := cHTMLHeader                                                                                                                                                                      ||
                ...
                'Hacemos de su conocimiento, que el pasado '||cTextoRojoOpen||OC_GENERALES.FECHA_EN_LETRA(TRUNC(dFechaCobro))||cTextoClose||' se realizó segundo intento de cobro a la cuenta '  ||
                'bancaria que nos proporcionó, desafortunadamente el banco nos notificó nuevamente"'||cTextoImportanteOpen||'transacción declinada por el emisor'||cTextoImportanteClose||'", '  ||
                ...
                'Centro de Servicios a Asegurados, el cual estará disponible en horario de oficina en los teléfonos (55) 44-33-89-00 opción 2, '                                                 ||
