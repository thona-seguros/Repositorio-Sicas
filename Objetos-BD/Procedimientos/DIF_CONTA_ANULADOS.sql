create or replace PROCEDURE DIF_CONTA_ANULADOS  IS
...
nCtaPrimaNeta      DETALLE_FACTURAS.Monto_Det_Moneda%TYPE;
...
nCtaDerechos       DETALLE_FACTURAS.Monto_Det_Moneda%TYPE;
-------------------------
...
cNomArchivo VARCHAR2(100) := 'DIF_CONTA_ANU_'||to_char(trunc(sysdate - 1),'ddmmyyyy')||'.XLSX';
...
cCodReporte           VARCHAR2(100) := 'DIFCONTAANULADOS';
...
cSubject                    VARCHAR2(1000) := 'Notificacion Archivo de Diferencias Contables Anulados ';
cTexto1                     VARCHAR2(10000):= 'Apreciable Compañero ';
cTexto2                     varchar2(1000)   := ' Envío archivo de Diferencias Contables Anulados generado de manera automática el día de hoy. ';
cTexto3                     varchar2(10)   := '  ';
...
CURSOR ANU_Q IS
   SELECT DISTINCT P.IdPoliza, P.NumPolUnico, /*DP.CodFilial,*/ OC_CLIENTES.NOMBRE_CLIENTE(P.CodCliente) || ' ' ||
          OC_FILIALES.NOMBRE_ADICIONAL(P.CodCia, P.CodGrupoEc, DP.CodFilial) Contratante, --F.IdEndoso,
          T.FechaTransaccion, P.Cod_Moneda, DP.IdTipoSeg, DP.PlanCob, PC.CodTipoPlan,
          OC_VALORES_DE_LISTAS.BUSCA_LVALOR('SUBRAMOS', PC.CodTipoPlan) DescSubRamo,
          P.CodCliente, P.FecIniVig, P.FecFinVig, P.NumRenov, P.CodCia, P.CodEmpresa, F.FecAnul,
          T.IdTransaccion
...
CURSOR DET_Q IS
   SELECT D.CodCpto, SUM(D.Monto_Det_Moneda) Monto_Det_Moneda, D.IndCptoPrima, C.IndCptoServicio
    ...
    WHERE C.CodConcepto        = D.CodCpto
      ...
      AND F.IdTransaccionAnu   = nIdTransaccion
      AND F.FecContabilizada  <= dFecHasta
      AND F.IndContabilizada   = 'S'
      AND F.StsFact            = 'ANU'
...
CURSOR FACT_Q IS
   SELECT IdPoliza, IdFactura, MtoComisi_Moneda
     FROM FACTURAS
    WHERE IdTransaccionAnu   = nIdTransaccion
      AND FecContabilizada  <= dFecHasta
...
CURSOR NC_Q IS
   SELECT DISTINCT P.IdPoliza, P.NumPolUnico, DP.CodFilial, OC_CLIENTES.NOMBRE_CLIENTE(P.CodCliente) || ' ' ||
          ...
          T.FechaTransaccion, P.Cod_Moneda, DP.IdTipoSeg, DP.PlanCob, PC.CodTipoPlan, DP.CodPlanPago,
          OC_VALORES_DE_LISTAS.BUSCA_LVALOR('SUBRAMOS', PC.CodTipoPlan) DescSubRamo, 
          P.CodCliente, P.FecIniVig, P.FecFinVig, P.NumRenov, P.CodCia, P.CodEmpresa,
          TRUNC(T.FechaTransaccion) FecAnul, T.IdTransaccion, N.MtoComisi_Moneda, N.IdEndoso,
...
CURSOR NC_DET_Q IS
   SELECT IdPoliza, IdNcr, MtoComisi_Moneda
     FROM NOTAS_DE_CREDITO
    WHERE (IdTransaccionAnu   = nIdTransaccion
       OR  IdTransaccion      = nIdTransaccion);
...
CURSOR DET_NC_Q IS
   SELECT D.CodCpto, D.Monto_Det_Moneda, D.IndCptoPrima, C.IndCptoServicio
    ...
    WHERE C.CodConcepto = D.CodCpto
      ...
      AND D.IdNcr       = N.IdNcr
      AND (IdTransaccionAnu   = nIdTransaccion
       OR  IdTransaccion      = nIdTransaccion);

CURSOR DET_ConC_NCR (nIdPoliza NUMBER, nIdNcr NUMBER) IS
   SELECT DCO.CodConcepto, ABS(DCO.Monto_Mon_Extranjera) Monto_Mon_Extranjera, AGE.CodTipo
     FROM COMISIONES COM, DETALLE_COMISION DCO, AGENTES AGE
    WHERE DCO.CodConcepto IN ('COMISI','COMIPF','COMIPM','HONORA','UDI')
      AND COM.CodCia       = DCO.CodCia 
      AND COM.IdComision   = DCO.IdComision
...
BEGIN
       ...
       VALUES(1,1,cCodReporte,trunc(sysdate),nLineaimp,'REPAUTDIFCONANU',cNomArchivo,cCadena);
...
BEGIN
   SELECT SYS_CONTEXT ('USERENV','CURRENT_USERID')
    ...
    SELECT SYS_CONTEXT('userenv', 'terminal'),
      ...
      VALUES('DIFERENCIAS CONTABLES ANULADOS',SYSDATE,W_ID_ENVIO,W_ID_USER,W_ID_TERMINAL);
      --
      COMMIT;
      --
...
--      nLinea := nLinea + 1;
      ...
      cTitulo1 := cNomCia;
      cTitulo2 := 'TRANSACCIONES DE ANULACION CON DIFERENCIAS CONTABLES DEL   '|| TO_CHAR(dFecDesde,'DD/MM/YYYY') || ' Al ' || TO_CHAR(dFecHasta,'DD/MM/YYYY');
      cTitulo4 := ' ';
      cEncabez    := 'No. de Póliza'||cLimitador||'Consecutivo'||cLimitador||'No. Endoso'||cLimitador||
                     'Sub-Grupo'||cLimitador||'Contratante'||cLimitador||'No. Transaccion'||cLimitador||
                     'Fecha Anulación'||cLimitador||'Prima Neta'||cLimitador||'6101.01.10/6107.00.10/6101.02.10'||cLimitador||
                     'Recargos'||cLimitador||'2704.00.10'||cLimitador||'Derechos'||cLimitador||'6501.01.10'||cLimitador||
                     'Impuesto'||cLimitador||'2607.00.10'||cLimitador||'Prima Total'||cLimitador||
                     '1601.01.10/1601.02.10/1604.01.10'||cLimitador||'Comisión Sobre Prima'||cLimitador||
                     'Comisión Persona Fisica'||cLimitador||'5303.01.10/5303.06.10/5306.04.10'||cLimitador||
                     'Comisión Persona Moral'||cLimitador|| '5305.01.10/5305.06.10/5305.04.10'||cLimitador||
                     'Honorarios Persona Fisica'||cLimitador||'Honorarios Persona Moral'||cLimitador||'5309.39.10'||cLimitador||
...
--         BEGIN
 ...
 --    nLinea := XLSX_BUILDER_PKG.EXCEL_DETALLE(nLinea + 1, cEncabez, 1);
   ...
   FOR X IN ANU_Q LOOP
      ...
      nCtaPrimaNeta      := OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '6', '1', '01', '01', '10') +
                            OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '6', '1', '05', '01', '10') +  --ESA 20171218 incluir prima unica
                            OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '6', '1', '07', '00', '10') +
                            OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '6', '1', '01', '02', '10');  --JMMD20200910                            
      ...
      nCtaComisionesPEF  := OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '5', '3', '03', '01', '10') +
                            OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '5', '3', '03', '06', '10') +
                            OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '5', '3', '03', '04', '10') ;  --JMMD20200910  
--                            OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '2', '3', '03', '01', '10'); --JMMD 20200923                                                              
      nCtaComisionesPEM  := OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '5', '3', '05', '01', '10') +
                            OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '5', '3', '05', '06', '10') +
                            OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '5', '3', '05', '04', '10') ; --JMMD20200910     
--                            OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '2', '3', '03', '02', '10'); --JMMD 20200923                             
      ...
      IF NVL(nCtaPrimaNeta,0) != NVL(nPrimaNeta,0) OR NVL(nCtaRecargos,0) != NVL(nRecargos,0) OR
         NVL(nCtaDerechos,0) != NVL(nDerechos,0) OR NVL(nCtaImpuesto,0) != NVL(nImpuesto,0) OR
         NVL(nCtaPrimaTotal,0) != NVL(nPrimaTotal,0) OR NVL(nCtaComisionesPEF,0) != NVL(nComisionesPEF,0) OR
         NVL(nCtaComisionesPEM,0) != NVL(nComisionesPEM,0) OR 
         NVL(nCtaHonorarios,0) != (NVL(nHonorariosPEF,0) + NVL(nHonorariosPEM,0)) OR
         NVL(nCtaUdis,0) != (NVL(nUdisPEF,0) + NVL(nUdisPEM,0)) THEN
...
--      IF :BK_DATOS.Formato = 'TEXTO' THEN
            cCadena := X.NumPolUnico                                  ||cLimitador||
                       ...
                       TO_CHAR(X.FecAnul,'DD/MM/YYYY')                ||cLimitador||
                       TO_CHAR(nPrimaNeta,'99999999999990.00')        ||cLimitador||
                       TO_CHAR(nCtaPrimaNeta,'99999999999990.00')     ||cLimitador||
                       ...
                       TO_CHAR(nCtaDerechos,'99999999999990.00')      ||cLimitador||
...
----------------------------------
   FOR X IN NC_Q LOOP
      ...
      cRecibos  := 'NC-';
      FOR Z IN NC_DET_Q  LOOP
         IF cRecibos = 'NC-' THEN
              cRecibos  := Z.IdNcr;
         ELSE
            cRecibos  := cRecibos || ' / ' || Z.IdNcr;
         END IF;
         nTotComision := NVL(nTotComision,0) + NVL(Z.MtoComisi_Moneda,0);
         FOR C IN DET_ConC_NCR (X.IdPoliza, Z.IdNcr) LOOP
            IF C.CodTipo = 'AGTEPF' THEN --AGENTE PERSONA FISICA
               IF C.CodConcepto IN ('COMISI','COMIPF') THEN
                  nComisionesPEF  := NVL(nComisionesPEF,0) + NVL(C.Monto_Mon_Extranjera,0);
               ELSIF C.CodConcepto = 'HONORA' THEN
...
EXCEPTION
   ...
   RAISE_APPLICATION_ERROR(-20000, 'Error en Generación de Transacciones de Anulación:  ' || SQLERRM);
END;