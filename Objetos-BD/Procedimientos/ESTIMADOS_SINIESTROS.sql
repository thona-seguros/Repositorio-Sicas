...
SELECT SI.IDPOLIZA,
       ...
       DECODE(NVL(D.MTOEXTRANERO,0),0,NVL(D.MTOLOCAL,0),NVL(D.MTOEXTRANERO,0)) MONTO_RESERVADO_MONEDA, --LARA 27/01/2026
       NVL(D.MTOLOCAL,0)            MONTO_RESERVADO_LOCAL, --LARA 27/01/2026
...
--          P.FORMAVENTA                                              CODCANALFORMAVENTA,
 ...
 WHERE PP.CODCIA     = nCodCia
   ...
   AND TRUNC(T.FECHATRANSACCION) >= TO_DATE('05/12/2025','DD/MM/YYYY') --DFECDESDE
   AND TRUNC(T.FECHATRANSACCION) <= TO_DATE('05/12/2025','DD/MM/YYYY') --DFECHASTA
   AND T.IDPROCESO                = 6
   --
...
CREATE OR REPLACE PUBLIC SYNONYM ESTIMADOS_SINIESTROS FOR SICAS_OC.ESTIMADOS_SINIESTROS;
/

GRANT EXECUTE ON SICAS_OC.ESTIMADOS_SINIESTROS TO PUBLIC;
/