...
cNomArchivo VARCHAR2(100) := 'PAGOS_PAG_'||to_char(trunc(sysdate - 1),'ddmmyyyy')||'.XLSX';
...
cTipoPago             APROBACION_ASEG.TIPO_APROBACION%TYPE := 'PAG';
cUsuario              VARCHAR2(8) := '%';
nCodCia               NUMBER := 1;
nCodEmpresa           NUMBER := 1;
cCodReporte           VARCHAR2(100) := 'APROBACIONESPAGADAS';
...
cSubject                    VARCHAR2(1000) := 'Notificacion Archivo de Aprobaciones Pagadas de Siniestros: ';
cTexto1                     VARCHAR2(10000):= 'Apreciable Compañero ';
cTexto2                     varchar2(1000)   := ' Envío archivo de Aprobaciones Pagadas de Siniestros generado de manera automática el día de hoy. ';
cTexto3                     varchar2(10)   := '  ';
...
BEGIN
   ...
   VALUES('APROBACIONESPAGADAS',SYSDATE,W_ID_ENVIO,W_ID_USER,W_ID_TERMINAL);
   --
   COMMIT;
   --
