...
--
CREATE OR REPLACE PROCEDURE SICAS_OC.SESASEMISIONVIGRP (nCodCia NUMBER, nCodEmpresa NUMBER, cCodEntrega VARCHAR2,
...
CURSOR POL_IND_Q IS
   SELECT P.NumPolUnico Poliza, 
    ...
    WHERE PN.Num_Doc_Identificacion  = A.Num_Doc_Identificacion
      ...
      AND D.IDetPol                  > 0
      AND D.IdPoliza                 > 0
      AND D.IdPoliza                IN (25614,25885,26245,26256,26445,26381,26377,26372,26316,26500,26459,26232,26535,26384,26455,26365,26410,26402.26525,26305,26262,26284,26340,26341,26403,26353,26374,26401,26421,26423,26424,26469,26513,26470,26514,26471,26515,26478,26491,26517, 26530,26533,26536,26539,26368);
...
CURSOR POL_COL_Q IS
   SELECT P.NumPolUnico Poliza,
          ...
          OC_PLAN_COBERTURAS.PLAN_POLIZA(D.CodCia, D.CodEmpresa, D.IdTipoSeg, D.PlanCob) PlanPoliza,
          PN.CodPaisRes CodPais, PN.CodProvRes CodEstado, P.IdPoliza, P.StsPoliza, P.FecIniVig,
          P.FecFinVig FecFinVigPol, D.IdTipoSeg, D.PlanCob, D.IDetPol, AC.Estado StsDetalle, D.FecFinVig,
          D.IndAsegModelo, NVL(D.CantAsegModelo,0) CantAsegModelo, AC.Cod_Asegurado, 'COL' TipoDetalle
