create or replace PROCEDURE DIF_CONTA_EMITIDOS  IS
...
cNomArchivo VARCHAR2(100) := 'DIF_CONTA_EMI_'||to_char(trunc(sysdate - 1),'ddmmyyyy')||'.XLSX';
...
cCodReporte           VARCHAR2(100) := 'DIFCONTAEMISION';
...
cSubject                    VARCHAR2(1000) := 'Notificacion Archivo de Diferencias Contables Emision ';
cTexto1                     VARCHAR2(10000):= 'Apreciable Compañero ';
cTexto2                     varchar2(1000)   := ' Envío archivo de Diferencias Contables Emisión generado de manera automática el día de hoy. ';
cTexto3                     varchar2(10)   := '  ';
cTexto4                     varchar2(1000)   := ' Saludos. ';
----

CURSOR EMI_Q IS 
SELECT IdPoliza,                 NumPolUnico, 
       Contratante,              FechaTransaccion, 
      IdTipoSeg,                PlanCob, 
      CodTipoPlan,              Cod_Moneda, 
       DescSubRamo,              CodCliente, 
      FecIniVig,                FecFinVig, 
      NumRenov,                 CodCia, 
      CodEmpresa,               IdTransaccion
FROM 
(   
   SELECT DISTINCT P.IdPoliza, P.NumPolUnico, /*DP.CodFilial,*/ OC_CLIENTES.NOMBRE_CLIENTE(P.CodCliente) || ' ' ||
          ...
          T.FechaTransaccion, DP.IdTipoSeg, DP.PlanCob, PC.CodTipoPlan, P.Cod_Moneda, --F.IdEndoso,
...
CURSOR DET_Q IS
   SELECT D.CodCpto, SUM(D.Monto_Det_Moneda) Monto_Det_Moneda, D.IndCptoPrima, C.IndCptoServicio
    ...
    WHERE C.CodConcepto       = D.CodCpto
      ...
      AND F.IndContabilizada  = 'S'
      AND ((nIdTransaccion    = F.IdTransaccion AND F.IdTransacContab IS NULL)
       OR  nIdTransaccion     = F.IdTransacContab)
    GROUP BY D.CodCpto, D.IndCptoPrima, C.IndCptoServicio;

CURSOR FACT_Q IS
   SELECT IdPoliza, IdFactura, MtoComisi_Moneda
     FROM FACTURAS
    WHERE IndContabilizada  = 'S'
      AND ((nIdTransaccion  = IdTransaccion AND IdTransacContab IS NULL)
       OR  nIdTransaccion   = IdTransacContab);
...
BEGIN
       ...
       VALUES(1,1,cCodReporte,trunc(sysdate),nLineaimp,'REPAUTDIFCONEMI',cNomArchivo,cCadena);
...
BEGIN
   SELECT SYS_CONTEXT ('USERENV','CURRENT_USERID')
    ...
    SELECT SYS_CONTEXT('userenv', 'terminal'),
      ...
      VALUES('DIFERENCIAS CONTABLES EMISION',SYSDATE,W_ID_ENVIO,W_ID_USER,W_ID_TERMINAL);
      --
      COMMIT;
      --
...
--      nLinea := nLinea + 1;
      ...
      cTitulo1 := cNomCia;
      cTitulo2 := 'TRANSACCIONES EMISION CON DIFERENCIAS CONTABLES DEL '|| TO_CHAR(dFecDesde,'DD/MM/YYYY') || ' Al ' || TO_CHAR(dFecHasta,'DD/MM/YYYY');
      cTitulo4 := ' ';
      cEncabez    := 'No. de Póliza'||cLimitador||'Consecutivo'||cLimitador||'No. Endoso'||cLimitador||
                     ...
                     'Fecha Emisión'||cLimitador||'Prima Neta'||cLimitador||'6101.01.10/6107.00.10'||cLimitador||
                     ...
                     'Comisión Persona Fisica'||cLimitador||'5303.01.10/5303.06.10/5303.04.10'||cLimitador||
                     'Comisión Persona Moral'||cLimitador|| '5303.01.10/5303.06.10/5303.04.10'||cLimitador||
 ...
 --    nLinea := XLSX_BUILDER_PKG.EXCEL_DETALLE(nLinea + 1, cEncabez, 1);
   ...
   FOR X IN EMI_Q LOOP
      ...
      nCtaPrimaNeta      := OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '6', '1', '01', '01', '10') +
                            OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '6', '1', '05', '01', '10') +  --ESA 20171219 Incluir prima unica
                            OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '6', '1', '07', '00', '10') +
                            OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '6', '1', '01', '02', '10') +  --JMMD 20200910
                            OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '6', '1', '05', '02', '10');  --JMMD 20200910                                                                                  
      ...
      nCtaPrimaTotal     := OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '1', '6', '01', '01', '10') +
                            ...
                            OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '1', '6', '03', '02', '10');   --ESA 20171219 Incluir prima unica
      nCtaComisionesPEF  := OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '5', '3', '03', '01', '10') +
                            ...
                            OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '5', '3', '03', '04', '10') ; --JMMD 20200910                          
--                            OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '2', '3', '03', '01', '10'); --JMMD 20200923                                                                                                              
      nCtaComisionesPEM  := OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '5', '3', '05', '01', '10') +
                            ...
                            OC_COMPROBANTES_DETALLE.SUMA_CUENTAS_CONTABLES(cNumComprob, '5', '3', '05', '04', '10') ; --JMMD 20200910
...
EXCEPTION
   ...
   RAISE_APPLICATION_ERROR(-20000, 'Error en Generación de Reporte de Diferencias Contables de Emisión' || SQLERRM);
END;