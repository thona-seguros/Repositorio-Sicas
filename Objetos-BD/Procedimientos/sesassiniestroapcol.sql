...
--
CREATE OR REPLACE PROCEDURE SICAS_OC.SESASSINIESTROAPCOL(nCodCia NUMBER, nCodEmpresa NUMBER, cCodEntrega VARCHAR2,
...
CURSOR C_ARCHIVO IS
   SELECT P.NumPolUnico Poliza, S.Cod_Asegurado Certi, C.TipoSeguro Tipo_Seg,
          ...
          DECODE(NVL(P.IndConcentrada,'N'),'N',0,1) PolConcentrada, D.IndAsegModelo, D.IDetPol
   ...
   SELECT P.NumPolUnico Poliza, S.Cod_Asegurado Certi, C.TipoSeguro Tipo_Seg,
          ...
          D.IdTipoSeg, D.IdPoliza, S.Monto_Pago_Moneda, S.Monto_Reserva_Moneda,
          DECODE(NVL(P.IndConcentrada,'N'),'N',0,1) PolConcentrada, D.IndAsegModelo, D.IDetPol
     FROM DETALLE_POLIZA D, POLIZAS P, ASEGURADO_CERTIFICADO AC, CLIENTES CL, PERSONA_NATURAL_JURIDICA PN,
          SINIESTRO S, CONFIG_SESAS_TIPO_SEGURO C
    WHERE PN.Num_Doc_Identificacion  = CL.Num_Doc_Identificacion
      AND PN.Tipo_Doc_Identificacion = CL.Tipo_Doc_Identificacion
      AND CL.CodCliente              = P.CodCliente
      AND AC.Cod_Asegurado           = S.Cod_Asegurado
      AND AC.IDetPol                 = S.IDetPol
      AND AC.IdPoliza                = S.IdPoliza
      AND AC.CodCia                  = S.CodCia
      AND C.IdTipoSeg                = D.IdTipoSeg
      AND C.CodEmpresa               = D.CodEmpresa
      AND C.CodCia                   = D.CodCia
      AND S.Fec_Notificacion        >= dFecDesde
...
BEGIN
   ...
   FOR X IN C_ARCHIVO LOOP
      ...
      IF X.PolConcentrada = 1 THEN
         IF X.IndAsegModelo = 'S' THEN
            IF X.Certi = 19781 THEN
               nCantCert := OC_DETALLE_POLIZA.TOTAL_ASEGURADOS(nCodCia, nCodEmpresa, X.IdPoliza, X.IDetPol);
               SELECT nCantCert - (COUNT(*) - 1)
                 INTO nCantCert
      ...
      nTotPolConce      := nTotPolConce + X.PolConcentrada;
   END LOOP;
   ...
   cCadena := TRIM(TO_CHAR(nTotPoliza,'99999999999999999990'))    || cSeparador ||
              ...
              '0'                                                 || cSeparador ||
              TRIM(TO_CHAR(nTotPolConce,'99999999999999999990'))  || cSeparador ||
   ...
   FOR X IN C_ARCHIVO LOOP
      ...
      cCadena := OC_ENTREGAS_CNSF_PLANTILLA.FORMATO_CAMPO(nCodCia, nCodEmpresa, cCodEntrega, cCodPlantilla,
                 ...
                 OC_ENTREGAS_CNSF_PLANTILLA.FORMATO_CAMPO(nCodCia, nCodEmpresa, cCodEntrega, cCodPlantilla,
                                                          OC_CONFIG_PLANTILLAS_CAMPOS.CAMPO(nCodCia, nCodEmpresa, cCodPlantilla, 26), X.PolConcentrada) || cSeparador ||
                 OC_ENTREGAS_CNSF_PLANTILLA.FORMATO_CAMPO(nCodCia, nCodEmpresa, cCodEntrega, cCodPlantilla,
                                                          OC_CONFIG_PLANTILLAS_CAMPOS.CAMPO(nCodCia, nCodEmpresa, cCodPlantilla, 27), cPuntoComa) || CHR(13);
      OC_ARCHIVO.Escribir_Linea(cCadena, cIdUsr, nLinea);
   END LOOP;
   OC_ARCHIVO.Escribir_Linea('EOF', cIdUsr, 0);
END SESASSINIESTROAPCOL;
/

--
-- SESASSINIESTROAPCOL  (Synonym) 
--
--  Dependencies: 
--   SESASSINIESTROAPCOL (Procedure)
--
CREATE OR REPLACE PUBLIC SYNONYM SESASSINIESTROAPCOL FOR SICAS_OC.SESASSINIESTROAPCOL
/


GRANT EXECUTE ON SICAS_OC.SESASSINIESTROAPCOL TO PUBLIC
/
