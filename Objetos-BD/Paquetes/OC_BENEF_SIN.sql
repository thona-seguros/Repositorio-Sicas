CREATE OR REPLACE PACKAGE          OC_BENEF_SIN IS
/*   _______________________________________________________________________________________________________________________________
	...
	| Nombre     : OC_BENEF_SIN                                                                                                     |
    | Objetivo   : Package para acciones detalladas de los Beneficiarios de Sinietros.                                              |
    |                                                                                                                               |
    | Modificado : Si                                                                                                               |
    | Ult. modif.: 19/07/2021                                                                                                       |
    | Modifico   : J. Alberto Lopez Valle [ JALV ]                                                                                  |
    | Para       : THONA Seguros                                                                                                    |    
    | Obj. Modif.: Se agrega funcionalidad para manejar Datos Bancarios Encriptados del Cliente y/o Agente segun sea el caso.       |
    |   Objetos afectados:                                                                                                          |
    |                       - INSERTA_ASEG_BENEF()                                                                                  |
    |                       - INSERTA_BENEF_PROV()                                                                                  |
    |   DEPENDENCIAS:                                                                                                               |
...
CREATE OR REPLACE PACKAGE BODY          OC_BENEF_SIN IS
--
--
--
FUNCTION NOMBRE_BENEFICIARIO(nIdSiniestro  NUMBER, nIdPoliza  NUMBER, nCod_Asegurado NUMBER,
...
dbms_output.put_line('Inserta Aseg IS NULL nBenef '||nBenef);
     BEGIN
       SELECT TRIM(PN.Nombre) Nombre, TRIM(PN.Apellido_Paterno) ApellPaterno, TRIM(PN.Apellido_Materno) ApellMaterno,
              ...
              PN.EMail, MP.CodEntidadFinan, MP.NumCuentaBancaria, MP.NumCuentaClabe --OC_GENERALES.DESENCRIPTAR_AES(MP.NumCuentaBancaria), OC_GENERALES.DESENCRIPTAR_AES(MP.NumCuentaClabe) -- MP.NumCuentaBancaria, MP.NumCuentaClabe  --> JALV (+) 19/07/2021
              ,PN.IDREGFISSAT
         INTO cNombre, cApellPaterno, cApellMaterno, dFecNac, cTipDocIdentif, cNumDocIdentif,
              cSexo, cDireccion, cEMail, cEntidad, cCuenta, cClabe, NIDREGFISSAT
         FROM SINIESTRO SI INNER JOIN  ASEGURADO AG              ON  SI.Cod_Asegurado           = AG.cod_asegurado                                                        
                           INNER JOIN PERSONA_NATURAL_JURIDICA PN ON PN.Tipo_Doc_Identificacion = AG.Tipo_Doc_Identificacion
                                                                 AND PN.Num_Doc_Identificacion  = AG.Num_Doc_Identificacion
                           LEFT JOIN  MEDIOS_DE_PAGO MP ON PN.Tipo_Doc_Identificacion = MP.Tipo_Doc_Identificacion
                                                       AND PN.Num_Doc_Identificacion  = MP.Num_Doc_Identificacion
                           LEFT JOIN  CAT_REGIMEN_FISCAL RF       ON PN.IDREGFISSAT = RF.IDREGFISSAT                                                                                
...
dbms_output.put_line('Inserta Aseg Nuevo nBenef '||nBenef);
     BEGIN
       ...
       VALUES(nIdSiniestro, nIdPoliza, nCod_Asegurado, nBenef, cNombre, 100, '0001', 'ACT', cSexo, TRUNC(SYSDATE), TRUNC(SYSDATE),
              'Pago por el Siniestro No. ' || nIdSiniestro, cDireccion, cEMail, cClabe, cEntidad, cCuenta, 'N', Null, --'Pago por el Siniestro No. ' || nIdSiniestro, cDireccion, cEMail, OC_GENERALES.ENCRIPTAR_AES(cClabe), cEntidad, OC_GENERALES.ENCRIPTAR_AES(cCuenta), 'N', Null,   -- cClabe, cCuenta, --> JALV (+) 19/07/2021
     ...
     BEGIN
       SELECT TRIM(PN.Nombre) Nombre, TRIM(PN.Apellido_Paterno) ApellPaterno, TRIM(PN.Apellido_Materno) ApellMaterno,
         ...
         INTO cNombre, cApellPaterno, cApellMaterno, dFecNac, cTipDocIdentif, cNumDocIdentif,
              ...
              NIDREGFISSAT
         FROM SINIESTRO SI INNER JOIN PERSONA_NATURAL_JURIDICA PN ON PN.Tipo_Doc_Identificacion = cTipo_Doc_Identificacion
     ...
     BEGIN
       INSERT INTO BENEF_SIN
             (IdSiniestro, IdPoliza, Cod_Asegurado, Benef, Nombre, PorcePart, CodParent, Estado, Sexo, FecEstado, FecAlta,
              ...
              FecNac, Tipo_Id_Tributario, Num_Doc_Tributario, Apellido_Paterno, Apellido_Materno)
       VALUES(nIdSiniestro, nIdPoliza, nCod_Asegurado, nBenef, cNombre, 100, '0001', 'ACT', cSexo, TRUNC(SYSDATE), TRUNC(SYSDATE),
              'Pago por el Siniestro No. ' || nIdSiniestro, cDireccion, cEMail, cClabe, cEntidad, cCuenta, 'N', Null, --'Pago por el Siniestro No. ' || nIdSiniestro, cDireccion, cEMail, OC_GENERALES.ENCRIPTAR_AES(cClabe), cEntidad, OC_GENERALES.ENCRIPTAR_AES(cCuenta), 'N', Null,   --cClabe, cEntidad, cCuenta,    --> JALV (-) 19/07/2021
              dFecNac, cTipDocIdentif, cNumDocIdentif, cApellPaterno, cApellMaterno);
     EXCEPTION
       WHEN OTHERS THEN
         RAISE_APPLICATION_ERROR(-20225,'Error al Insertar Beneficiario de Pago del Siniestro a partir del Asegurado '|| nIdSiniestro || ' ' || SQLERRM);
