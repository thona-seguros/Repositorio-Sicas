create or replace PACKAGE          OC_FACTURAS IS
    ...
    --
    FUNCTION MONTO_BASE_IMPUESTO( nCodCia       NUMBER, nIdFactura    NUMBER, cCodTipoPlan VARCHAR2) RETURN NUMBER;
    --
...
create or replace PACKAGE BODY          OC_FACTURAS IS
    ...
    */
          ...
          SELECT
              CASE
                  ...
                  WHEN EXISTS (
                       SELECT 1 FROM TEMP_RECIBOS_PROV
                       WHERE IdPoliza       = W.IdPoliza
                         AND IdFacturaIni   = nIdFacturaAnu
                  ) THEN 'S2'
                  ELSE NULL
              END AS INDICADOR
          INTO cIndicador
    ...
    CURSOR CONCEN_Q IS
       SELECT IdFondo, IdMovimiento, CodCptoMov, MontoMovMoneda
        ...
        WHERE CodCia        = nCodCia
          ...
          AND StsMovimiento IN ('SOLICI', 'ACTIVO')  --MLJS 14/10/2023  SE AGREGO EL ESTADO ACTIVO
          AND GT_FAI_MOVIMIENTOS_FONDOS.TIPO_MOVIMIENTO(CodCia, CodEmpresa, cTipoFondo, CodCptoMov) IN ('AA','AP');
    BEGIN
       BEGIN
          SELECT F.CodCia, F.Saldo_Moneda, F.Saldo_Local, F.Monto_Fact_Moneda,
    ...
    --
    FUNCTION MONTO_BASE_IMPUESTO( nCodCia       NUMBER, nIdFactura    NUMBER, cCodTipoPlan VARCHAR2) RETURN NUMBER IS
    ...
    BEGIN
       SELECT SUM(Monto_Det_Moneda)
        ...
        WHERE IdFactura  = nIdFactura
          ...
          AND OC_CATALOGO_DE_CONCEPTOS.RAMO_REAL(nCodCia, CodCpto) = NVL(cCodTipoPlan,'NA');
       RETURN nMontoBase;
    ...
    --
    FUNCTION MONTO_BASE(nIdFactura NUMBER,  cIndExentoImp VARCHAR2 := NULL) RETURN NUMBER IS
        nMonto_Det_Moneda    DETALLE_FACTURAS.Monto_Det_Moneda%TYPE;
    BEGIN
       SELECT --DECODE(NVL(MTOIMPTOFACTELECT, 0), 0, 'S', 'N') ExentoImp,
              SUM(Monto_Det_Moneda)
         INTO nMonto_Det_Moneda
         FROM DETALLE_FACTURAS
        WHERE IdFactura  = nIdFactura
          AND OC_CATALOGO_DE_CONCEPTOS.INDICADOR_CONCEPTO(1, CodCpto, 'IMPUESTO') = 'N'
          AND DECODE(NVL(MTOIMPTOFACTELECT, 0), 0, 'S', 'N') = cIndExentoImp;
     --GROUP BY DECODE(NVL(MTOIMPTOFACTELECT, 0), 0, 'S', 'N');
    ...
    --
    PROCEDURE INSERT_TEMP_RECIBOS_PROV(nCodCia NUMBER, nCodEmpresa NUMBER, nIdFacturaAnu NUMBER, nIdPoliza NUMBER) IS
    cIndicador    VARCHAR2(2);
    cExiste       VARCHAR2(2);
    CURSOR RECB1_Q IS
       SELECT *
