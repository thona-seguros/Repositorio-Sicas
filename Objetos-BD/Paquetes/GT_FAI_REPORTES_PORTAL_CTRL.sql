CREATE OR REPLACE PACKAGE GT_FAI_REPORTES_PORTAL_CTRL AS
...
/
create or replace PACKAGE BODY  GT_FAI_REPORTES_PORTAL_CTRL AS
   ...
   BEGIN
      SELECT 'S'
       ...
       WHERE CodCia                       = nCodCia
         ...
         --AND NVL(MesReporte,1)   = NVL(cMesReporte, 1)      --> JALV(+) 13/08/2021
         --AND NVL(AnioReporte,1)  = NVL(cAnioReporte, 1);    --> JALV(+) 13/08/2021
    ...
    FOR X IN POLIZAS_Q LOOP      
       ...
       IF cExisteReporte = 'S' THEN
            FOR J IN REPORTES_Q LOOP
                ...
                ELSIF J.Reporte = 'ESTCTAFO' /*AND cIndGeneraEdoCta = 'S'*/ THEN        --cTipoReporte:= '002';                                 
                    FOR W IN PARAMREP_Q LOOP
                        IF W.Nom_Param LIKE '%CODCIA%' THEN
                           cValorParam := cValorParam||'                                           XMLELEMENT("'||W.Nom_Param||'", '||TRIM(TO_CHAR(nCodCia))     ||'), '|| CHR(10);
                        ELSIF W.Nom_Param LIKE '%IDPOLIZA%' THEN
                ...
                ELSIF J.Reporte = 'APORTREG' /*AND GT_FAI_REPORTES_PORTAL_CTRL.EXISTE_REPORTE(nCodCia, nCodEmpresa, X.CodCliente, X.IdPoliza, '009', NULL, NULL) = 'N'*/ THEN   --cTipoReporte:= '009';                                       
                    FOR W IN PARAMREP_Q LOOP
                        IF W.Nom_Param LIKE '%CODCIA%' THEN
                           cValorParam := cValorParam||'                                           XMLELEMENT("'||W.Nom_Param||'", '||TRIM(TO_CHAR(nCodCia))     ||'), '|| CHR(10);
                        ELSIF W.Nom_Param LIKE '%POLIZA%' THEN
                ...
                ELSIF J.Reporte = 'APORTEXT' /*AND GT_FAI_REPORTES_PORTAL_CTRL.EXISTE_REPORTE(nCodCia, nCodEmpresa, X.CodCliente, X.IdPoliza, '010', NULL, NULL) = 'N'*/ THEN   --cTipoReporte:= '010';                                       
                    FOR W IN PARAMREP_Q LOOP
                        IF W.Nom_Param LIKE '%CODCIA%' THEN
                           cValorParam := cValorParam||'                                           XMLELEMENT("'||W.Nom_Param||'", '||TRIM(TO_CHAR(nCodCia))     ||'), '|| CHR(10);
                        ELSIF W.Nom_Param LIKE '%POLIZA%' THEN
...
CURSOR POLIZAS_Q IS
    ...
    AND     OC_TIPOS_DE_SEGUROS.MANEJA_FONDOS(P.CodCia, P.CodEmpresa, DP.IdTipoSeg)                                         = 'S'  
    AND     GT_FAI_FONDOS_DETALLE_POLIZA.EXISTEN_FONDOS(P.CodCia, P.CodEmpresa, P.IdPoliza, DP.IDetPol, DP.Cod_Asegurado)   = 'S'
    AND     P.FecFinVig >= TRUNC(SYSDATE)   --fecha hoy
    AND     TO_CHAR(P.FecFinVig,'DD') = TO_CHAR(SYSDATE,'DD') -- MESVERSARIO !!!
   -- AND     P.IdPoliza   =  39904--38523--38782 --nIdPoliza
    --AND     P.NumPolUnico = 'VF0110-30913-01'
    --AND     P.FecFinVig > TRUNC(SYSDATE)--fecha hoy
    ;
...
BEGIN   
    FOR x IN POLIZAS_Q LOOP
        ...
        nIdPoliza        := x.IdPoliza;
          dFecMesAniversario := TRUNC(SYSDATE);--TRUNC(ADD_MONTHS( x.FecIniVig, 1));

        SELECT  TO_CHAR(dFecMesAniversario, 'MM'), TO_CHAR(dFecMesAniversario, 'YYYY')
        INTO    cMesEdoCta, cAnioEdoCta
        FROM    DUAL;

        cExisteReporte := GT_FAI_REPORTES_PORTAL_CTRL.EXISTE_REPORTE(nCodCia, nCodEmpresa, x.CodCliente, x.IdPoliza, '002', cMesEdoCta, cAnioEdoCta);

          IF cExisteReporte = 'N' /*AND (dFecMesAniversario + 1) <= TRUNC(SYSDATE)*/ THEN
            dFecIni          := ADD_MONTHS(dFecMesAniversario, -1);
            dFecFin          := dFecMesAniversario;
            cIndGeneraEdoCta := 'S';
            --EXIT;
       ...
       END IF;       
        FOR J IN REPORTES_Q LOOP
            nIdReporte  := J.IdReporte;
            IF J.Reporte = 'POLIZAVF' AND GT_FAI_REPORTES_PORTAL_CTRL.EXISTE_REPORTE(nCodCia, nCodEmpresa, x.CodCliente, x.IdPoliza, '001', NULL, NULL) = 'N' THEN --cMesEdoCta, cAnioEdoCta) = 'N' THEN
                cTipoReporte:= '001';                
            ...
            ELSIF J.Reporte = 'ESTCTAFO' AND GT_FAI_REPORTES_PORTAL_CTRL.EXISTE_REPORTE(nCodCia, nCodEmpresa, x.CodCliente, x.IdPoliza, '002', cMesEdoCta, cAnioEdoCta) = 'N' /*AND cIndGeneraEdoCta = 'S'*/ AND TO_CHAR(TRUNC(SYSDATE),'DD') = cDiaIniVig THEN
               cTipoReporte:= '002';                  
                        GT_FAI_REPORTES_PORTAL_CTRL.INSERTAR(nCodCia, nCodEmpresa, x.CodCliente, x.IdPoliza, cTipoReporte, TRUNC(SYSDATE), TO_CHAR(SYSDATE, 'HH:MI:SS'), TO_CHAR(dFecMesAniversario, 'MM'), TO_CHAR(dFecMesAniversario, 'RRRR'), J.Reporte, NULL, NULL);                                                          
...
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Problema detectado en la Poliza: '||nIdPoliza||', reporte: '||nIdReporte||', Tipo de Reporte: '||cTipoReporte||' y Mesversario : '||dFecMesAniversario||'; Error: '||SQLERRM);
