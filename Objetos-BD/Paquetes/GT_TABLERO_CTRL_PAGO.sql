CREATE OR REPLACE PACKAGE          GT_TABLERO_CTRL_PAGO IS

    NumMaxLote Number := 200;
    NumRegLote Number := 0;
  ...
  FUNCTION NUMERO_REGISTROS_POR_LOTE RETURN NUMBER;
...
/
create or replace PACKAGE BODY          GT_TABLERO_CTRL_PAGO IS
    ...
    FUNCTION NUMERO_REGISTROS_POR_LOTE RETURN NUMBER AS
        NUMMAXLOTE NUMBER;
    BEGIN
        BEGIN
            SELECT TO_NUMBER(DESCVALLST)
             ...
             WHERE CODLISTA = 'NUMMAXLOTE'
               AND CODVALOR = 'NUMREG';

        EXCEPTION WHEN OTHERS THEN
               NUMMAXLOTE := 9999999999;
        END;
        --
        RETURN NUMMAXLOTE;
        --
    END NUMERO_REGISTROS_POR_LOTE;
    ...
    PROCEDURE EJECUTA_RESPUESTA (pCodCia NUMBER := 1, pDesde VARCHAR2 :=NULL, pHasta VARCHAR2 := NULL, pFolio_Solicitud NUMBER := null) IS
        ...
        nReg                 NUMBER :=0;
    BEGIN 
        FOR TAB IN (  SELECT DISTINCT T.CODCIA, T.IDPROC, DECODE(pFolio_Solicitud, NULL, NULL, D.IDPROCDET) IDPROCDET, D.TIPO_PAGO
            ...
            FOR ENT IN (  SELECT D.IDPROCDET, D.FOLIO_SOLICITUD, D.REFERENCIA, D.FECHA, D.FACTURA, TIPO_PAGO
                ...
                BEGIN
                    ...
                    GT_DETALLE_TABLERO_CTRL_PAGO.ACTUALIZA_PAGO(TAB.CODCIA
                                                               ...
                                                               ,NVL(GT_WEB_SERVICES.ExtraeDatos_XmlDom('tipo_pago'), ENT.TIPO_PAGO)                                                              
                                                               );
                    IF MOD(nReg, 100) = 0 THEN
                        COMMIT;
                    END IF;             

                    IF INSTR(GT_WEB_SERVICES.ExtraeDatos_XmlDom('referencia'), 'SINIESTRO INFOACOT') > 0 THEN -- ACTUALIZA EL DETALLE DE LAS APROBACIONES DE SINIESTROS PARA INDICAR EL LOTE QUE PERTENECE
    ...
    PROCEDURE CREA_LOTE_FONDEO_COMISIONES(PCODCIA NUMBER, PFECHA DATE, PFECHA2 DATE,PIDPROC OUT NUMBER) IS 
        ...
        CURSOR QRY_COM IS
                SELECT  COMPANIA,
                        ...
                        TIPO_CAMBIO,
                        TASA_IVA,
                        CASE WHEN INSTR(CLASIF_PAGO, 'AGTE') > 0 THEN 
                                  TOTAL_MO + (IMP_RET_IVA*-1) + (IMP_RET_ISR*-1) + (IVA_MO * -1)  
                             ELSE SUB_TOTAL_MO 
                        END SUB_TOTAL_MO,
                        IVA_MO,
                        TOTAL_MO,
                ...
                FROM (
                            SELECT  
                                    ...
                                    SUM(DN.Monto_Det_Moneda )                                                               sub_total_mo,
                                    0                                                                                       iva_mo,       
                                    SUM(DN.Monto_Det_Moneda )                                                               total_mo,     
                                    OC_CATALOGO_DE_CONCEPTOS.PORCENTAJE_CONCEPTO(NC.CODCIA, 'RETIVA')                       tasa_ret_iva, 
                                    0                                                                                       imp_ret_iva,  
                                    OC_CATALOGO_DE_CONCEPTOS.PORCENTAJE_CONCEPTO(NC.CODCIA, 'RETISR')                       tasa_ret_isr, 
                                    0                                                                                       imp_ret_isr,  
                                    (SELECT MAX(CD.CodCentroCosto)   FROM  COMPROBANTES_DETALLE CD WHERE  CD.CodCia = nCodCia AND CD.NumComprob = CC.NumComprob AND CD.NIVELAUX = NVL(NC.CTALIQUIDADORA,0)  ) ccs,
                                    (SELECT MAX(CD.CodUnidadNegocio) FROM  COMPROBANTES_DETALLE CD WHERE  CD.CodCia = nCodCia AND CD.NumComprob = CC.NumComprob AND CD.NIVELAUX = NVL(NC.CTALIQUIDADORA,0) ) uen,
                                     '*'                                                                    pys,          
                                    'COMMAS03'                                                              concepto,     
                                    ...
                                    1                                                   plaza_cta,    
                                    DECODE(A.codformapago, 'TRS', 0, 'CLAB', 0, 1)      tipo_pago,   --0 es TRANSFERENCIA, 1 ES CHEQUE
                                    NULL REFERENCIA_02,
                                    NULL REFERENCIA_03,
                                    1                                                   fuente,       
                                    ''                                                  folio_solicitud,
                    ...
                    GROUP BY NC.CODCIA,
                             ...
                             AGE.CODTIPO                                
                     order by TO_CHAR(TRUNC(NC.FecDevol),'YYYY/MM/DD'), NC.IDNOMINA );
    ...
    BEGIN
        ...
        NumRegLote := 0;
        NumMaxLote := NUMERO_REGISTROS_POR_LOTE;
        --
        FOR ENT IN QRY_COM LOOP

            IF NumRegLote = NumMaxLote THEN
                GT_TABLERO_CTRL_PAGO.ACTUALIZA_RESPUESTA(nCodCia, nIdProc);    
                nSwLote  := 0;
                NumRegLote := 1;
            ELSE
                NumRegLote := NumRegLote + 1; 
            END IF; 
 ...
 PROCEDURE CREA_LOTE_FONDEO_SINIEST_IND(PCODCIA NUMBER, PFECHA DATE, PFECHA2 DATE,PIDPROC OUT NUMBER) IS 
        ...
        SELECT /*+ RULE +*/ -- T.IDTRANSACCION NUM_TRANSACCION, , 
         ...
         NVL((SELECT E.CODBCOMIZAR FROM ENTIDAD_FINANCIERA E WHERE e.codcia = nCodCia AND E.CODENTIDAD = NVL(BES.ENT_FINANCIERA , decode(NVL(BES.NUMCUENTABANCARIA,BES.CUENTA_CLAVE) , null, 0, '4000'))), BES.ENT_FINANCIERA) CTA_BANCO,
   ...
   WHERE TRUNC(T.FECHATRANSACCION) >= to_date(dFecDesde,'dd/mm/yyyy') --to_date('01/02/2018','dd/mm/yyyy')        --dFecDesde  
    ...
    PROCEDURE CREA_LOTE_FONDEO_INFONACOT(PCODCIA NUMBER, PFECHA DATE, PIDPROC OUT NUMBER) IS 
        ...
        CURSOR QRY_SIN IS
                    ...
                    ,'*'                                                            pys        
                    ,'SINTS01'                                                      concepto        
                    ,'Siniestro Infonacot(WS.NoReg:' || COUNT(*) ||')-No:' ||I.REF_BANCA_PAGO REFERENCIA     
                    ,11                                                             grupo        
                    ,8                                                              flujo        
                    ,I.ID_ENVIO                                                     factura
        ...
        CURSOR QRY_SIN IS
            SELECT  IDPOLIZA                                                     POLIZA,
                    ...
                    5                                                            compania,
                     ...
                     8                                                                      FLUJO, 
                     NUMCOMPROB                                                             FACTURA,
                     (SELECT E.CODBCOMIZAR FROM ENTIDAD_FINANCIERA E WHERE E.CODENTIDAD = CTA_BANCO AND E.CODCIA = nCodCia) id_cta_banco,                     
                     CLAVE                                                                  CLABE,
                     1                                                                      PLAZA_CTA,
                     TIPO_PAGO                                                              TIPO_PAGO,      
                     NULL                                                                   REFERENCIA_02,
            ...
            FROM (                        
                    ...
                    WHERE T.CodCia                  = PCODCIA 
                      ...
                      AND TRUNC(T.FechaTransaccion) BETWEEN  PFECHA AND PFECHA2 
                      AND T.IdProceso               = 6                                             
                      ...
                      AND A.IdTransaccion           = D.IdTransaccion 
                      ...
                      AND SI.IdSiniestro     = A.IdSiniestro
                      AND C.CODCIA           = A.CODCIA                      
                      AND C.NUMTRANSACCION   = T.IDTRANSACCION
                      AND BES.CODCIA         = A.CODCIA
                      AND BES.CODEMPRESA     = A.CODEMPRESA
                      AND BES.IDSINIESTRO    = A.IDSINIESTRO
                      AND BES.IDPOLIZA       = A.IDPOLIZA
                      AND BES.BENEF          = A.BENEF 
                      AND DS.CODCIA          = A.CODCIA
                      AND DS.CODEMPRESA      = A.CODEMPRESA
                      AND DS.IdSiniestro            = SI.IdSiniestro
                      AND DS.IdDetSin               = A.IdDetSin
                      AND NOT EXISTS (SELECT 1 
                                        FROM DETALLE_TABLERO_CTRL_PAGO P 
                                       WHERE P.CODCIA = C.CODCIA AND P.FACTURA = C.NUMCOMPROB)                              
                    UNION                                
                  SELECT /*+ RULE +*/ T.IdTransaccion, SI.IdPoliza, 
                    ...
                    WHERE T.CodCia                  = PCODCIA 
                     ...
                     AND T.USUARIOGENERO NOT IN (  SELECT CODUSUARIO 
                      ...
                      AND A.StsAprobacion          IN ('PAG','ANU')
                      AND A.Num_Aprobacion          > 0
                      AND SI.CODCIA                 = T.CODCIA
                      AND SI.CODEMPRESA             = T.CODEMPRESA
                      AND SI.IdSiniestro            = A.IdSiniestro
                      AND DS.CODCIA                 = T.CODCIA
                      AND DS.CODEMPRESA             = T.CODEMPRESA
                      AND DS.IdSiniestro            = SI.IdSiniestro
                      AND DS.IdDetSin               = A.IdDetSin
                      AND DS.Cod_Asegurado          = SI.Cod_Asegurado
                      AND C.CODCIA                  = T.CODCIA                      
                      AND C.NUMTRANSACCION          = T.IDTRANSACCION
                      AND BES.CODCIA                = T.CODCIA
                      AND BES.CODEMPRESA            = T.CODEMPRESA
                      AND BES.IDSINIESTRO           = A.IDSINIESTRO
                      AND BES.IDPOLIZA              = A.IDPOLIZA
                      AND BES.BENEF                 = A.BENEF 
                      ...
                      AND TS.IDTIPOSEG              = DS.IDTIPOSEG
                      AND PMS.CodCia           (+) = A.CODCIA
                      AND PMS.CodEmpresa       (+) = A.CODEMPRESA
