...
--   SPLIT_TBL (Type)
--
CREATE OR REPLACE PACKAGE SICAS_OC.zip_util_pkg
is
  /*
  ...
  EJEMPLO DE USO:
    -- EN EL EJEMPLO UNE UN UN ZIP VARIOS ARCHIVOS, GENERA EL ARCHIVO Y ELIMINA LOS ARCHIVOS DE ENTRADA
    BEGIN
        IF zip_util_pkg.ZIP_ARCHIVOS('DIR_TMP', 'ARCHIVOZIP', 'SalidaExcelDeclar.xls,SalidaExcelDeclar2.xls') THEN
            DBMS_OUTPUT.PUT_LINE('SE GENERO EL ZIP');
        END IF;
    END;    
...
--
  ...
  --
  FUNCTION file_to_blob (DIRECTORIO VARCHAR2, p_file_name VARCHAR2) RETURN BLOB;
  --
  FUNCTION ZIP_ARCHIVOS(DIRECTORIO           VARCHAR2,  -- Directorio definido en oracle para depositar el archivo zipeado
                      NOM_ARCH_SALIDA_ZIP   VARCHAR2,  --nombre del archivo de salida del zip
                      NOM_ARCHIVOS          VARCHAR2)  -- nombre de los archivos a zipear, separados por coma(,) 
                      RETURN BOOLEAN;
  --
end zip_util_pkg;
/
...
--
-- ZIP_UTIL_PKG  (Package Body) 
--
--  Dependencies: 
--   ZIP_UTIL_PKG (Package)
--
CREATE OR REPLACE PACKAGE BODY SICAS_OC.zip_util_pkg
is
...
--
  ...
  begin
    ...
    --
    FUNCTION file_to_blob (DIRECTORIO VARCHAR2, p_file_name VARCHAR2) RETURN BLOB AS
        dest_loc BLOB := EMPTY_BLOB ();
        src_loc BFILE := BFILENAME (DIRECTORIO, p_file_name);
    BEGIN
    ...
    BEGIN
        ...
        FOR ENT IN (SELECT COLUMN_VALUE ARCHIVO
                     from table(GT_WEB_SERVICES.split(NOM_ARCHIVOS))) LOOP
            l_file := zip_util_pkg.file_to_blob (DIRECTORIO, ENT.ARCHIVO);
            zip_util_pkg.add_file (l_zip, ENT.ARCHIVO, l_file);
        END LOOP;
        --
        zip_util_pkg.finish_zip (l_zip);
        --  
        IF INSTR(UPPER(NOM_ARCH_SALIDA_ZIP), '.ZIP')> 0 THEN
            NOM_ARCHIVO_ZIP := UPPER(NOM_ARCH_SALIDA_ZIP);
        ELSE
            NOM_ARCHIVO_ZIP := UPPER(NOM_ARCH_SALIDA_ZIP) || '.ZIP';
        END IF;
        --
        zip_util_pkg.save_zip (l_zip, DIRECTORIO, NOM_ARCHIVO_ZIP);
        --
        BEGIN
            FOR ENT IN (SELECT COLUMN_VALUE ARCHIVO
                         from table(GT_WEB_SERVICES.split(NOM_ARCHIVOS))) LOOP
                UTL_FILE.FREMOVE(DIRECTORIO, ENT.ARCHIVO);                     
            END LOOP;
    ...
    EXCEPTION WHEN OTHERS THEN
        RAISE_APPLICATION_ERROR (-20001,'Error en la generación del ZIP: '|| sqlerrm);
        RETURN FALSE;    
    END ZIP_ARCHIVOS;
...
--
-- ZIP_UTIL_PKG  (Synonym) 
--
--  Dependencies: 
--   ZIP_UTIL_PKG (Package)
--
CREATE OR REPLACE PUBLIC SYNONYM ZIP_UTIL_PKG FOR SICAS_OC.ZIP_UTIL_PKG
/


GRANT EXECUTE ON SICAS_OC.ZIP_UTIL_PKG TO PUBLIC
/
