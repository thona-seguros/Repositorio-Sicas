...
PROCEDURE CC_CARGA (
  ...
  P_MENSAJE      OUT VARCHAR2) IS
   ...
   CURSOR CurCC IS
       SELECT *
       FROM   RESERVA_DET
       WHERE  FE_MOVTO     >= P_FECHA_INICIAL
...
CURSOR INICIAL IS
    ...
    WHERE ((P_ID_SINIESTRO <> 0 AND DT.VALOR1 = P_ID_SINIESTRO)
      ...
      AND  EXISTS (  
       ...
       AND  T.IDPROCESO     = 6 
       AND  T.CODCIA        = P_CODCIA
       AND  T.CODEMPRESA    = P_CODCIA
...
CURSOR TRAN IS
   ...
   FROM (    
        ...
        AND    CC.CODCIA         = P_CODCIA
        AND    CC.NUMTRANSACCION = DT.IDTRANSACCION
        AND    CD.CODCIA         = P_CODCIA
...
EXCEPTION
   WHEN OTHERS THEN
      ...
      RAISE_APPLICATION_ERROR(-20230,'Error en : '||V_SINIESTRO||' '||P_MENSAJE);
...
BEGIN
   ...
   END; 
    ...
    REC_CTROL_RVA.TOT_REGS_RI    := CASE WHEN NVL(PTOT_REGS_RI,0)    = 0 THEN 0 ELSE PTOT_REGS_RI    END;
    REC_CTROL_RVA.IMPTE_AJUMAS   := CASE WHEN NVL(PIMPTE_AJUMAS,0)   = 0 THEN 0 ELSE PIMPTE_AJUMAS   END;
    REC_CTROL_RVA.TOT_REGS_AA    := CASE WHEN NVL(PTOT_REGS_AA,0)    = 0 THEN 0 ELSE PTOT_REGS_AA    END;
    REC_CTROL_RVA.IMPTE_AJUMENOS := CASE WHEN NVL(PIMPTE_AJUMENOS,0) = 0 THEN 0 ELSE PIMPTE_AJUMENOS END;
    REC_CTROL_RVA.TOT_REG_AD     := CASE WHEN NVL(PTOT_REG_AD,0)     = 0 THEN 0 ELSE PTOT_REG_AD     END;
    REC_CTROL_RVA.IMPTE_PAGOS    := CASE WHEN NVL(PIMPTE_PAGOS,0)    = 0 THEN 0 ELSE PIMPTE_PAGOS    END;
    REC_CTROL_RVA.TOT_REGS_PA    := CASE WHEN NVL(PTOT_REGS_PA,0)    = 0 THEN 0 ELSE PTOT_REGS_PA    END;
    REC_CTROL_RVA.IMPTE_DESPAGOS := CASE WHEN NVL(PIMPTE_DESPAGOS,0) = 0 THEN 0 ELSE PIMPTE_DESPAGOS END;
    REC_CTROL_RVA.TOT_REGS_DESP  := CASE WHEN NVL(PTOT_REGS_DESP,0)  = 0 THEN 0 ELSE PTOT_REGS_DESP  END;
...
--
-- TH_CARGA_OPC  (Synonym) 
--
--  Dependencies: 
--   TH_CARGA_OPC (Package)
--
CREATE OR REPLACE PUBLIC SYNONYM TH_CARGA_OPC FOR SICAS_OC.TH_CARGA_OPC
/


GRANT EXECUTE ON SICAS_OC.TH_CARGA_OPC TO PUBLIC
/
