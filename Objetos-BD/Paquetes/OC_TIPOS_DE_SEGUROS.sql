CREATE OR REPLACE PACKAGE OC_TIPOS_DE_SEGUROS IS
...
-- HOMOLOGACION VIFLEX                                                       2022/03/01  JMMD
  ...
  FUNCTION CLAVE_FACT_ELECTRONICA(nCodCia NUMBER, nCodEmpresa NUMBER, cIdTipoSeg VARCHAR2, cCodTipoPlan VARCHAR2 := NULL) RETURN VARCHAR2;
  ...
  FUNCTION INDMULTIRAMO(PCODCIA NUMBER, PCODEMPRESA NUMBER, PIDTIPOSEG VARCHAR2) RETURN VARCHAR2;

END OC_TIPOS_DE_SEGUROS;

/

CREATE OR REPLACE PACKAGE BODY OC_TIPOS_DE_SEGUROS IS
    --
...
-- HOMOLOGACION VIFLEX                                                           2022/03/01  JMMD    
    ...
    CURSOR TIPOSEG_Q IS
      SELECT TipoSeg, IdPlantilla, CodTipoPlan, DiasCancelacion,
             ...
             ClaveFactElect, IndManejaFondos, T.INDMULTIRAMO
       ...
       FOR X IN TIPOSEG_Q LOOP
          INSERT INTO TIPOS_DE_SEGUROS
                 (CodCia, CodEmpresa, IdTipoSeg, Descripcion, StsTipSeg, FecSts,
                  ...
                  TipoContabilidad, DiasCancelacion, ClaveFactElect, IndManejaFondos, INDMULTIRAMO)
          VALUES (nCodCia, nCodEmpresa, cIdTipoSegDest, cDescripDest, 'ACT', TRUNC(SYSDATE),
                  ...
                  X.TipoContabilidad, X.DiasCancelacion, X.ClaveFactElect, X.IndManejaFondos, x.INDMULTIRAMO);
    ...
    FUNCTION CLAVE_FACT_ELECTRONICA(nCodCia NUMBER, nCodEmpresa NUMBER, cIdTipoSeg VARCHAR2, cCodTipoPlan VARCHAR2 := NULL) RETURN VARCHAR2 IS
    ...
    BEGIN
       BEGIN
          SELECT ClaveFactElect
            ...
            FROM (
                  SELECT NVL(ClaveFactElect, '<<N/D>>')     ClaveFactElect
                    FROM TIPOS_DE_SEGUROS
                   WHERE CodCia     = nCodCia
                     AND CodEmpresa = nCodEmpresa
                     AND IdTipoSeg  = cIdTipoSeg
                     AND cCodTipoPlan IS NULL
                  UNION ALL
                  SELECT L.DESCVALLST ClaveFactElect
                   FROM SICAS_OC.VALORES_DE_LISTAS L
                  WHERE CODLISTA = 'CVEFACTELE'
                    AND CODVALOR = cCodTipoPlan
                    AND cCodTipoPlan IS NOT NULL
                );
    ...
    FUNCTION INDMULTIRAMO(PCODCIA NUMBER, PCODEMPRESA NUMBER, PIDTIPOSEG VARCHAR2) RETURN VARCHAR2 IS
        VALOR VARCHAR2(1);
    BEGIN
          SELECT 'S'
            INTO VALOR
            FROM TIPOS_DE_SEGUROS T
           WHERE T.IDTIPOSEG  = PIDTIPOSEG
             AND T.CODEMPRESA = PCODEMPRESA
             AND T.CODCIA     = PCODCIA
             AND T.INDMULTIRAMO ='S';
        RETURN VALOR;
