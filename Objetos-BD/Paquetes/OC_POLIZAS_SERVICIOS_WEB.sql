create or replace PACKAGE OC_POLIZAS_SERVICIOS_WEB AS
   ...
   FUNCTION GENERA_POLIZA( nCodCia        NUMBER
    ...
    --
    FUNCTION ANULAR_POLIZA(nCodCia    NUMBER,   nCodEmpresa NUMBER,   nIdPoliza NUMBER,      dFecAnul DATE,
...
*/
    ...
    */
    ...
    --    
    FUNCTION ANULAR_POLIZA(nCodCia    NUMBER,   nCodEmpresa NUMBER,   nIdPoliza NUMBER,                       dFecAnul DATE,
                           cMotivAnul VARCHAR2, cCod_Moneda VARCHAR2, cRespuesta OUT VARCHAR2, cTipoProceso VARCHAR2 DEFAULT 'POLIZA') RETURN Number IS
    --
        nEjecucion NUMBER(1) := 0;    
        cUrl       VARCHAR2(32724);
        cIP        VARCHAR2(100) := sys_context('userenv','server_host') ;
    --
    BEGIN
        --
        BEGIN
            ...
            --
            BEGIN
                OC_POLIZAS.ANULAR_POLIZA(nCodCia   , nCodEmpresa, nIdPoliza,   dFecAnul,
                                         cMotivAnul, cCod_Moneda, cTipoProceso);
                --
                FOR ENT IN (SELECT  N.CODCIA,
                               MIN(IDENDOSO)   P_ENDOSOINI, 
                               MAX(IDENDOSO)   P_ENDOSOFIN, 
                               MIN(N.IDETPOL)  P_IDETPOLINI, 
                               MAX(N.IDETPOL)  P_IDETPOLFIN, 
                               N.IDPOLIZA P_IDPOLIZA, 
                               N.IDPOLIZA P_POLIZA
                          FROM NOTAS_DE_CREDITO N INNER JOIN  DETALLE_TRANSACCION  T ON N.CODCIA   = T.CODCIA 
                                                                                    AND N.IDNCR    = T.VALOR4 
                                                                                    AND N.IDENDOSO = T.VALOR3 
                                                                                    AND N.STSNCR = 'EMI'
                        WHERE T.CODSUBPROCESO = 'NOTACR'
                          AND T.CODCIA =  nCodCia
                          AND T.CODEMPRESA = nCodEmpresa
                          AND T.VALOR1 =  nIdPoliza  
                          AND TRUNC(N.FECDEVOL) = TRUNC(dFecAnul)
                        GROUP BY  N.CODCIA,
                               N.IDETPOL,   
                               N.IDPOLIZA) LOOP
                    --
                    IF length(cUrl) > 0 THEN
                        cUrl := cUrl || CHR(10);
                    END IF;
                    --
                    /*           
                    cUrl := cUrl || 'http://' || cIP || ':8889' || '/reports/rwservlet?report=AVIDEVOL.rep&destype=cache&desformat=pdf&userid=sicas_oc/sicas@desarrollo&paramform=no' ||
                            '|P_CODCIA='    || ENT.CODCIA       || 
                            '|P_ENDOSOFIN=' || ENT.P_ENDOSOFIN  || 
                            '|P_ENDOSOINI=' || ENT.P_ENDOSOINI  || 
                            '|P_IDETPOLFIN='|| ENT.P_IDETPOLFIN || 
                            '|P_IDETPOLINI='|| ENT.P_IDETPOLINI || 
                            '|P_IDPOLIZA='  || ENT.P_IDPOLIZA   || 
                            '|P_POLIZA='    || ENT.P_POLIZA;
                    */                                                      
                    cUrl := cUrl || 'AVIDEVOL'            ||
                            '|'  || ENT.P_ENDOSOFIN ; 
--                            '|'  || ENT.P_ENDOSOINI  || 
...
--                            '|'  || ENT.P_POLIZA;
                ...
                END LOOP;                    
                --
                FOR ENT IN (SELECT CODCIA, IDENDOSO, IDPOLIZA
                              FROM ENDOSOS
                             WHERE CODCIA = nCodCia
                               AND IDPOLIZA = nIdPoliza
                               AND IDENDOSO = IDENDOSO
                               AND STSENDOSO = 'EMI'
                               AND MOTIVO_ENDOSO = 'SOLCON'
                               AND TRUNC(FECINIVIG) = TRUNC(dFecAnul)) LOOP
                    --
                    IF length(cUrl) > 0 THEN
                        cUrl := cUrl || CHR(10);
                    END IF;
                    --
                    /*                    
                    cUrl := cUrl || 'http://' || cIP || ':8889' || '/reports/rwservlet?report=ENDOSOS.rep&destype=cache&desformat=pdf&userid=sicas_oc/sicas@desarrollo&paramform=no' ||
...
--                            '|'  || ENT.IDPOLIZA;
            ...
            END;
            --
            --  keyncr: destype=cache desformat=pdf userid=thonapi/Th0n4p13v0luc10n@producci paramform=no server=rwterminal report=%1 P_CODCIA=1 P_ENDOSOFIN=%2 P_ENDOSOINI=%3 P_IDETPOLFIN=%4 P_IDETPOLINI=%5 P_IDPOLIZA=%6 P_POLIZA=%7 %1 %2 %3 %4 %5 %6 %7
            -- keyeno: destype=cache desformat=pdf userid=thonapi/Th0n4p13v0luc10n@producci paramform=no server=rwterminal report=%1 P_CODCIA=1 P_IDENDOSO=%2 P_POLIZA=%3 %1 %2 %3 
            -- http://192.168.0.232:8889/reports/rwservlet?report=AVIDEVOL.rep&destype=cache&desformat=pdf&userid=sicas_oc/sicas@desarrollo&paramform=no&P_CODCIA=1&P_ENDOSOFIN=0&P_ENDOSOINI=0&P_IDETPOLFIN=1&P_IDETPOLINI=1&P_IDPOLIZA=35497&P_POLIZA=35497
