...
CREATE OR REPLACE PACKAGE SICAS_OC.OC_POLIZAS IS
    ...
    FUNCTION RENOVAR(nCodCia NUMBER, nIdPolizaRen NUMBER, cEmitePoliza VARCHAR2 DEFAULT 'S') RETURN NUMBER;
   ...
   FOR R IN AGT_Q LOOP
      FOR T IN DET_Q LOOP
         ...
         SELECT NVL(SUM(Porc_Com_Proporcional),0), NVL(SUM(Porc_Com_Distribuida),0), NVL(SUM(Porc_Comision_Agente),0)
           INTO nPorc_Com_Proporcional, nPorc_Com_Distribuida, nPorc_Comision_Agente
         ...
         IF NVL(R.Porc_Com_Proporcional,0) != nPorc_Com_Proporcional THEN
            RAISE_APPLICATION_ERROR(-20200,'Comisión Proporcional del ' || NVL(R.Porc_Com_Proporcional,0) ||
                                    ...
                                    nPorc_Com_Proporcional || '% a Nivel Póliza');
...
FUNCTION RENOVAR(nCodCia NUMBER, nIdPolizaRen NUMBER, cEmitePoliza VARCHAR2 DEFAULT 'S') RETURN NUMBER IS
dFecHoy          DATE;
dFecFin          DATE;
dFecIni          DATE;
nIdPoliza        POLIZAS.IdPoliza%TYPE;
...
FUNCTION TOTAL_ASEGURADOS(nCodCia NUMBER, nCodEmpresa NUMBER, nIdPoliza NUMBER) RETURN NUMBER IS
...
BEGIN
  ...
  UPDATE COMPROBANTES_CONTABLES
     SET TIPOCOMPROB = 'PRE'     -- CANCELA CONTABILIDAD
