CREATE OR REPLACE PACKAGE          OC_DETALLE_APROBACION_ASEG IS
...
END OC_DETALLE_APROBACION_ASEG;
/

CREATE OR REPLACE PACKAGE BODY          OC_DETALLE_APROBACION_ASEG IS

    FUNCTION NUMERO_DETALLE(nIdSiniestro NUMBER, nNum_Aprobacion NUMBER) RETURN NUMBER IS
    nIdDetAprob        DETALLE_APROBACION_ASEG.IdDetAprob%TYPE;
    BEGIN
    ...
    BEGIN
       BEGIN
          nIdDetAprob := OC_DETALLE_APROBACION_ASEG.NUMERO_DETALLE(nIdSiniestro, nNum_Aprobacion);
       ...
       EXCEPTION
          WHEN OTHERS THEN
             RAISE_APPLICATION_ERROR(-20225,'INSERTA DETALLE_APROBACION_ASEG - Error: '||SQLERRM);
       END;
    END INSERTA_DETALLE;
    ...
    nMonto_Local        DETALLE_APROBACION_ASEG.Monto_Local%TYPE;
    BEGIN
    ...
    nMonto_Moneda        DETALLE_APROBACION_ASEG.Monto_Moneda%TYPE;
    BEGIN
        IF cCodCptoTransac IS NULL THEN
    ...
    cCodTransac        DETALLE_APROBACION_ASEG.CodTransac%TYPE;
    BEGIN
       BEGIN
          SELECT CodTransac
            INTO cCodTransac
    ...
    BEGIN
        ...
        nTasaCambio := NVL(OC_GENERALES.TASA_DE_CAMBIO(cCod_Moneda, TRUNC(DFECHACAMB)), 1);
      ...
      */
      ...
      BEGIN
        OC_DETALLE_APROBACION_ASEG.INSERTA_DETALLE(nIdSiniestro, NNUMAPROB, CCOD_PAGO, NISRLOCAL, NISR, CCODTRANSAC, CCODCPTOTRANSAC);
      END;

      nMonto_Moneda := NVL(nMonto,0) + nISR; 

      nMonto_Local := (NVL(nMonto,0) + nISR) * nTasaCambio;
