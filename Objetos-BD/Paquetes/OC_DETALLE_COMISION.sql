...
AND A.CODCIA = C.CODCIA
AND A.CODEMPRESA = C.CODEMPRESA
AND A.CODTIPO IN('HONPF','HONORF') --,'HONPM') --MLJS 21/02/2024
AND RCC.CODCIA = C.CODCIA
AND RCC.CODEMPRESA = C.CODEMPRESA
AND RCC.IDTIPOSEG = cIdTipoSeg
...
AND A.CODCIA = C.CODCIA
AND A.CODEMPRESA = C.CODEMPRESA
--AND A.CODTIPO IN('HONPF','HONORF') --,'HONPM') --MLJS 21/02/2024 SE AGREGA EL TIPO HONORF
AND A.CODTIPO IN('HONORF') --MLJS 03/05/2024
AND RCC.CODCIA = C.CODCIA
AND RCC.CODEMPRESA = C.CODEMPRESA
AND RCC.IDTIPOSEG = cIdTipoSeg
AND RCC.CODCONCEPTO = 'RETISR'
--AND RCC.CODTIPO IN('HONPF','HONORF') --,'HONPM')--MLJS 21/02/2024 SE AGREGA EL TIPO HONORF
AND RCC.CODTIPO IN('HONORF') --MLJS 03/05/2024
AND RCC.ORIGEN = C.ORIGEN
AND RCC.CODTIPOPLAN = '099'
GROUP BY  C.CodCia , C.IdComision , C.IDFACTURA, C.IDNCR, C.ORIGEN, C.COD_MONEDA, C.COD_AGENTE, A.CODTIPO,
 RCC.CodConcepto
...
AND A.CODCIA = C.CODCIA
AND A.CODEMPRESA = C.CODEMPRESA
AND A.CODTIPO IN('HONPF','HONORF') --,'HONPM')  --MLJS 21/02/2024 SE AGREGO EL TIPO HONORF
AND RCC.CODCIA = C.CODCIA
AND RCC.CODEMPRESA = C.CODEMPRESA
AND RCC.IDTIPOSEG = cIdTipoSeg
