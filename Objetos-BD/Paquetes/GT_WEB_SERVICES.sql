...
create or replace PACKAGE BODY SICAS_OC.GT_WEB_SERVICES IS
        ...
        ELSE
            ...
            IF dbms_lob.getlength(cRespStrXML) > 0 AND esDatosXMLResp = 'N' THEN  --RESPUESTA                
                ...
                exception when others then
                    ...
                    raise_application_error(-20010, 'No hay Respuesta del WS, posiblemente el periodo o el envío no hay datos a procesar' || CHR(10) || SQLERRM);                                        
                END;
            END IF;
    ...
    BEGIN
            ...
            RETURN  cDoc;
    EXCEPTION WHEN OTHERS THEN
        RETURN  xmltype(nvl(cLinXml,'<?xml version="1.0" encoding="utf-8"?><ERROR>Revise la configuración de los atributos, pudierá falta atributos del nombre del espacio</ERROR>'));
    END GENERA_XML;
    --
    ...
    BEGIN
        SELECT TRANSLATE(upper(pCadena1), 'ñáéíóúàèìòùãõâêîôôäëïöüçÑÁÉÍÓÚÀÈÌÒÙÃÕÂÊÎÔÛÄËÏÖÜÇ,.-<>;:_{}[]+*~^´¨¿¡\?=)(/&%$#"!|°¬','naeiouaeiouaoaeiooaeioucNAEIOUAEIOUAOAEIOOAEIOUC') 
          INTO Cadena1
        from DUAL;
        SELECT TRANSLATE(upper(pCadena2), 'ñáéíóúàèìòùãõâêîôôäëïöüçÑÁÉÍÓÚÀÈÌÒÙÃÕÂÊÎÔÛÄËÏÖÜÇ,.-<>;:_{}[]+*~^´¨¿¡\?=)(/&%$#"!|°¬','naeiouaeiouaoaeiooaeioucNAEIOUAEIOUAOAEIOOAEIOUC') 
          INTO Cadena2
        from DUAL;
