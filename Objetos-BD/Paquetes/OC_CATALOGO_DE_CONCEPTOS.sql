CREATE OR REPLACE PACKAGE OC_CATALOGO_DE_CONCEPTOS IS
...
FUNCTION RAMO_REAL(nCodCia NUMBER, cCodConcepto VARCHAR2) RETURN VARCHAR2;

END OC_CATALOGO_DE_CONCEPTOS;
/

CREATE OR REPLACE PACKAGE BODY OC_CATALOGO_DE_CONCEPTOS IS
...
FUNCTION RAMO_REAL(nCodCia NUMBER, cCodConcepto VARCHAR2) RETURN VARCHAR2 IS
cCodTipoPlan CATALOGO_DE_CONCEPTOS.CodTipoPlan%TYPE;
BEGIN
   BEGIN
      SELECT NVL(CodTipoPlan,'NA')
        INTO cCodTipoPlan
        FROM CATALOGO_DE_CONCEPTOS
       WHERE CodCia      = nCodCia
         AND CodConcepto = cCodConcepto;
