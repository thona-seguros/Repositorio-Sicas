CREATE OR REPLACE PACKAGE          oc_generales IS
--
...
CREATE OR REPLACE PACKAGE BODY          oc_generales IS
...
END CODCIA_USUARIO;
    ...
    --
    FUNCTION VALIDA_FECHA_PAGO(pReciboPago varchar2, FecPago DATE) RETURN DATE IS
        FecFmt      VARCHAR2(50);
        ReciboPago VARCHAR2(50);
        Sw          NUMBER(1) := 0;
        FechaOk     DATE;
        --
        FUNCTION ESVALIDA(Entrada Varchar2, Formato Varchar2) return boolean is
           FECHA DATE;
        BEGIN
            FECHA := TO_DATE(ENTRADA, FORMATO);
            RETURN TRUE;
    ...
    BEGIN
        ...
        IF (INSTR(pReciboPago, '-') > 0 
               or INSTR(pReciboPago, '.') > 0 
               or INSTR(pReciboPago, '#') > 0 
               or INSTR(pReciboPago, ' ') > 0) 
            AND INSTR(pReciboPago, '/') > 0 THEN
            ReciboPago := replace(replace(replace(replace(pReciboPago, '-', '!'), '.', '!'), '#', '!'), ' ', '!');
            ReciboPago := substr(ReciboPago, INSTR(ReciboPago, '!')+1);
        ELSE
            ReciboPago := pReciboPago;
        END IF;

        ReciboPago := replace(ReciboPago, ' ', '');

        IF LENGTH(ReciboPago) = 8 THEN  --DDMMYYYY
           FecFmt := SUBSTR(ReciboPago, 1,2) || '/' || SUBSTR(ReciboPago, 3,2) || '/' || SUBSTR(ReciboPago, 5,4); 
        ELSIF LENGTH(ReciboPago) = 6 THEN  --DDMMYY
           FecFmt := SUBSTR(ReciboPago, 1,2) || '/' || SUBSTR(ReciboPago, 3,2) || '/' || SUBSTR(ReciboPago, 5,2);
        ELSE
           FecFmt := ReciboPago;
        END IF;

        FecFmt := REPLACE(FecFmt, '-', '/');

        CASE WHEN ESVALIDA(FecFmt,'DD/MM/YYYY') THEN
                FechaOk := TO_DATE(FecFmt,'DD/MM/YYYY');
           ELSE CASE WHEN ESVALIDA(FecFmt,'DD/MM/YY') THEN
                    FechaOk := TO_DATE(FecFmt,'DD/MM/YYYY');
                ELSE CASE WHEN ESVALIDA(FecFmt,'YYYY/MM/DD') THEN
                        FechaOk := TO_DATE(FecFmt,'YYYY/MM/DD');
                    ELSE CASE WHEN ESVALIDA(FecFmt,'YY/MM/DD') THEN
                            FechaOk := TO_DATE(FecFmt,'YY/MM/DD');
                         ELSE
                            FechaOk := FecPago;                   
                    END CASE;                        
                END CASE;
        ...
        END CASE;
        --
        IF FecPago IS NOT NULL THEN
            IF FechaOk IS NOT NULL THEN
                RETURN FechaOk;              
            ELSE
                RETURN FecPago;
            END IF;
        ELSIF FechaOk IS NOT NULL THEN
            RETURN FechaOk;
        ELSE 
            RETURN NULL;
        END IF;         
