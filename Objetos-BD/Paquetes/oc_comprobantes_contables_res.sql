--
-- OC_COMPROBANTES_CONTABLES_RES  (Package) 
--
--  Dependencies: 
--   STANDARD (Package)
--   STANDARD (Package)
...
--   TRANSACCION (Table)
--
CREATE OR REPLACE PACKAGE SICAS_OC.oc_comprobantes_contables_RES IS
  ...
  PROCEDURE ACTUALIZA_ENVIO(nCodCia NUMBER, nNumComprob NUMBER);
...
END OC_COMPROBANTES_CONTABLES_RES;
/

--
-- OC_COMPROBANTES_CONTABLES_RES  (Package Body) 
--
--  Dependencies: 
--   OC_COMPROBANTES_CONTABLES_RES (Package)
--
CREATE OR REPLACE PACKAGE BODY SICAS_OC.oc_comprobantes_contables_RES IS
...
PROCEDURE ACTUALIZA_ENVIO(nCodCia NUMBER, nNumComprob NUMBER) IS
BEGIN
   BEGIN
      UPDATE COMPROBANTES_CONTABLES
         SET StsComprob  = 'ENV',
...
BEGIN
   ...
   cContabilizo   := 'N';
DBMS_OUTPUT.PUT_LINE('OC_COMPROBANTES_CONTABLES_RES INICIA');
DBMS_OUTPUT.PUT_LINE(' nCodCia        -> '||nCodCia);
DBMS_OUTPUT.PUT_LINE(' nIdTransaccion -> '||nIdTransaccion);
DBMS_OUTPUT.PUT_LINE(' cTipoComp      -> '||cTipoComp);
...
DBMS_OUTPUT.PUT_LINE('  cDescMovGeneral   -> '||cDescMovGeneral);
         ...
         FOR W IN MOV_Q LOOP
            ...
            cCodCpto       := W.CodCpto;
DBMS_OUTPUT.PUT_LINE('   nCodEmpresa   -> '||nCodEmpresa);
DBMS_OUTPUT.PUT_LINE('   cIdTipoSeg    -> '||cIdTipoSeg);
DBMS_OUTPUT.PUT_LINE('   cCodCpto      -> '||cCodCpto);
...
DBMS_OUTPUT.PUT_LINE('     X.TipoRegistro    -> '||X.TipoRegistro);
DBMS_OUTPUT.PUT_LINE('     X.TipoAgente      -> '||X.TipoAgente);
DBMS_OUTPUT.PUT_LINE('     W.MtoMovCuenta    -> '||W.MtoMovCuenta);
DBMS_OUTPUT.PUT_LINE('     X.TipoAgente      -> '||X.TipoAgente);
DBMS_OUTPUT.PUT_LINE('     W.MtoComisCuenta  -> '||W.MtoComisCuenta);
DBMS_OUTPUT.PUT_LINE('     X.TipoPersona     -> '||X.TipoPersona);
DBMS_OUTPUT.PUT_LINE('     X.CanalComisVenta -> '||X.TipoPersona);
...
DBMS_OUTPUT.PUT_LINE('nMtoMovCuentaLocal - >'||nMtoMovCuentaLocal);
                  OC_COMPROBANTES_DETALLE_RES.INSERTA_DETALLE(nCodCia, nNumComprob, X.NivelCta1, X.NivelCta2,
...
--
-- OC_COMPROBANTES_CONTABLES_RES  (Synonym) 
--
--  Dependencies: 
--   OC_COMPROBANTES_CONTABLES_RES (Package)
--
CREATE OR REPLACE PUBLIC SYNONYM OC_COMPROBANTES_CONTABLES_RES FOR SICAS_OC.OC_COMPROBANTES_CONTABLES_RES
/
