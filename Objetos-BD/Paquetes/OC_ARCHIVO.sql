CREATE OR REPLACE PACKAGE SICAS_OC.OC_ARCHIVO IS
  ...
  PROCEDURE ACTUALIZA_ARCHIVO(cCodUser VARCHAR2, cFormato VARCHAR2, cNomArchivo VARCHAR2);
END OC_ARCHIVO;
...
PROCEDURE ACTUALIZA_ARCHIVO(cCodUser VARCHAR2, cFormato VARCHAR2, cNomArchivo VARCHAR2) IS
BEGIN
   UPDATE TEMP_GEN_ARCHIVO
      SET Formato       = cFormato,
          NombreArchivo = cNomArchivo
    WHERE Coduser       = cCodUser
      AND Formato       IS NULL
