create or replace PACKAGE           API_GENERALES AS
/******************************************************************************
...
END API_GENERALES;
/
create or replace PACKAGE BODY  API_GENERALES AS
    ...
    BEGIN
        ...
        END IF;
        --Solo se actualiza la ocupacion si tiene datos para evitar sobreescritura de vacíos
...
--
    ...
    BEGIN
            SELECT count(*) INTO nNumCotizaciones
                ...
                --AQUÃ¿ ES DONDE SE REALIZARA LA DIVISIÃ“N DE LAS PÃ¿GINAS
   ...
   --
    PROCEDURE API_MUESTRA_COTIZACIONES(nIdCotizacion IN NUMBER, nCodAgente IN NUMBER, cResult OUT CLOB) IS
        pEstatus        VARCHAR2(10):= NULL;   --Todos los estatus
        --pEstatus in 'EMITID'  CotizaciÃ³n solo EMITIDA
        --            'POLEMI', CotizaciÃ³n con Poliza emitida
        --------------para paginaciÃ³n
        nNumRegIni      NUMBER := 1;    --Para todos los registros entonces 0 (cero)
        nNumRegFin      NUMBER := 50;   --todos los registros entonces 0, recomendaciÃ³n de 50 en 50 la paginaciÃ³n como mÃ¡ximo
        --------------------------------     
    BEGIN
...
PROCEDURE API_PERSONA_NATURAL_JURIDICA (IDPOL IN NUMBER, SetXml OUT CLOB) IS
   ...
   END LOOP; 
      --OBTENER LAS CANTIDADES DE COTIZACIONES A ELIMINAR CON EL PARÃ¿METRO O CONDICIÃ“N DE QUE SU VIGENCIA DE COTIZACIÃ“N YA HAYA CADUCADO 
  ...
  END PA_CONSULTAR_FEC_POLIZAS_REN;
    ...
    BEGIN
       ...
       INSERT INTO API_LOG_EMISION (DESCRIPCION, FECHA,ID_COTIZACION) VALUES(cadena, sysdate,nIdCotizacion);
        ...
        IF (nIdPoliza = 0) THEN
            nSetXml := THONAPI.GENERALES_PLATAFORMA_DIGITAL.PRE_EMITE_POLIZA_NEW_CFDI40(nCodCia, nCodEmpresa, nIdCotizacion, cCadena, NULL, xInformacionFiscal);
            --nSetXml := GENERALES_PLATAFORMA_DIGITAL.PRE_EMITE_POLIZA_NEW(nCodCia, nCodEmpresa, nIdCotizacion, cCadena, NULL);
        END IF;
        IF (nIdPoliza != 0) THEN
            nSetXml := THONAPI.GENERALES_PLATAFORMA_DIGITAL.PRE_EMITE_POLIZA_NEW_CFDI40(nCodCia, nCodEmpresa, nIdCotizacion, cCadena, nIdPoliza, xInformacionFiscal);
            --nSetXml := GENERALES_PLATAFORMA_DIGITAL.PRE_EMITE_POLIZA_NEW(nCodCia, nCodEmpresa, nIdCotizacion, cCadena, nIdPoliza);
        END IF;
        --PRE_EMITE_POLIZA_NEW(nCodCia, nCodEmpresa, nIdCotizacion, cCadena, cIdpOLIZA);
