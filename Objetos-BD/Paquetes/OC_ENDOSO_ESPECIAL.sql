create or replace PACKAGE          OC_ENDOSO_ESPECIAL IS
/***********************************************************************************/
...
/***********************************************************************************/
   ...
   PROCEDURE INSERTA_ENDOSO_TIPOADMI(NIDPOLIZA NUMBER, nIDetPol NUMBER,CUSUARIO VARCHAR2, CTERMINAL VARCHAR2, CTIPOENDOSO VARCHAR2, NVALORANT VARCHAR2, NVALORNUE VARCHAR2);
   
   PROCEDURE INSERTA_ENDOSO_AGRUPADOR(NIDPOLIZA NUMBER, nIDetPol NUMBER,CUSUARIO VARCHAR2, CTERMINAL VARCHAR2, CTIPOENDOSO VARCHAR2, NVALORANT VARCHAR2, NVALORNUE VARCHAR2);

END OC_ENDOSO_ESPECIAL;

/

create or replace PACKAGE BODY          OC_ENDOSO_ESPECIAL IS
  ...
  PROCEDURE INSERTA_ENDOSO_TIPOADMI(NIDPOLIZA NUMBER, nIDetPol NUMBER,CUSUARIO VARCHAR2, CTERMINAL VARCHAR2, CTIPOENDOSO VARCHAR2, NVALORANT VARCHAR2, NVALORNUE VARCHAR2)  IS -- Este es para hacer modificaciones especiales y generar el endoso
    ...
    BEGIN
      UPDATE POLIZAS SET  TIPOADMINISTRACION = NVL(NVALORNUE,TIPOADMINISTRACION)
      WHERE  IDPOLIZA = NIDPOLIZA;
    EXCEPTION
      WHEN OTHERS THEN
        RAISE_APPLICATION_ERROR(-20200,'Error al actualizar la póliza.');
        --RAISE eERROR;
    END;

    CDESCENDOSO:='SE CREO ENDOSO SIN VALOR PARA EL CAMBIO DE TIPO DE ADMINISTRACION EL DIA,'|| SYSTIMESTAMP ||'POR EL USUARIO, '|| CUSUARIO ||', EN LA TERMINAL - '||CTERMINAL;
    ...
    values              (NIDPOLIZA  ,nIdEndoso,'ESV'    ,NNUMENDREF,dFecIniVig,dFecFinVig,TRUNC(SYSDATE) ,TRUNC(SYSDATE),'EMI'     ,TRUNC(SYSDATE),
                         null       ,null     ,null     ,0              ,0               ,0               ,CDESCENDOSO  ,0        ,
                         0        , 1         ,1      ,nIDetPol,null  ,CTIPOENDOSO   ,NULL,'N',null,null,null,null,null,null,null);
   ...
   -- END IF;
    ...
    IF NVALORNUE IS NOT NULL THEN
       NCORRELATIVO := OC_ENDOSO_ESPECIAL.OBTENER_CORRELATIVO(NIDPOLIZA);
       OC_ENDOSO_ESPECIAL.INSERTA_DETALLE_ENDOSO_SV(NIDPOLIZA, nIdEndoso, nIDetPol, NCORRELATIVO, 'TIPOADMINISTRACION', NVALORANT, NVALORNUE,'POLIZAS', 'Tipo de Administracion');
    END IF;
  ...
  EXCEPTION
    WHEN  OTHERS THEN
        RAISE_APPLICATION_ERROR(-20200,'ERROR AL CAMBIAR EL TIPO DE ADMINISTRACIÓN, NIDPOLIZA:' || NIDPOLIZA || ' nIDetPol: ' || nIDetPol|| ' CUSUARIO: ' || CUSUARIO|| ' CTERMINAL: ' || CTERMINAL|| ' CTIPOENDOSO: ' || CTIPOENDOSO|| ' NVALORANT: ' || NVALORANT|| ' NVALORNUE: ' || NVALORNUE || SQLERRM);

  END INSERTA_ENDOSO_TIPOADMI;
   
PROCEDURE INSERTA_ENDOSO_AGRUPADOR(NIDPOLIZA NUMBER, nIDetPol NUMBER,CUSUARIO VARCHAR2, CTERMINAL VARCHAR2, CTIPOENDOSO VARCHAR2, NVALORANT VARCHAR2, NVALORNUE VARCHAR2)  IS 
    ...
    BEGIN
      UPDATE POLIZAS SET  CODAGRUPADOR = NVL(NVALORNUE,CODAGRUPADOR)
      WHERE  IDPOLIZA = NIDPOLIZA;
    EXCEPTION
      WHEN OTHERS THEN
        RAISE_APPLICATION_ERROR(-20200,'Error al actualizar la póliza.');
    ...
    CDESCENDOSO:='SE CREO ENDOSO SIN VALOR PARA EL CAMBIO DE AGRUPADOR EL DIA,'|| SYSTIMESTAMP ||'POR EL USUARIO, '|| CUSUARIO ||', EN LA TERMINAL - '||CTERMINAL;
   ...
   -- END IF;
    ...
    IF NVALORNUE IS NOT NULL THEN
       NCORRELATIVO := OC_ENDOSO_ESPECIAL.OBTENER_CORRELATIVO(NIDPOLIZA);
       OC_ENDOSO_ESPECIAL.INSERTA_DETALLE_ENDOSO_SV(NIDPOLIZA, nIdEndoso, nIDetPol, NCORRELATIVO, 'CODAGRUPADOR', NVALORANT, NVALORNUE,'POLIZAS', 'Agrupador');
    END IF;
  ...
  EXCEPTION
    WHEN  OTHERS THEN
        RAISE_APPLICATION_ERROR(-20200,'ERROR AL CAMBIAR EL AGRUPADOR, NIDPOLIZA:' || NIDPOLIZA || ' nIDetPol: ' || nIDetPol|| ' CUSUARIO: ' || CUSUARIO|| ' CTERMINAL: ' || CTERMINAL|| ' CTIPOENDOSO: ' || CTIPOENDOSO|| ' NVALORANT: ' || NVALORANT|| ' NVALORNUE: ' || NVALORNUE || SQLERRM);

  END INSERTA_ENDOSO_AGRUPADOR;
