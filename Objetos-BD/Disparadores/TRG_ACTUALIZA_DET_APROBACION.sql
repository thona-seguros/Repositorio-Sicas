  ...
  CREATE OR REPLACE TRIGGER "SICAS_OC"."TRG_ACTUALIZA_DET_APROBACION" 
 BEFORE UPDATE ON DETALLE_APROBACION
...
BEGIN
  ...
  VALUES(nMaxtransaccion, 1, 'DETALLE_APROBACION ' || :NEW.IdSiniestro, SYSDATE, cIP,
         :NEW.IdSiniestro||'-'||:NEW.Num_Aprobacion||'-'||:NEW.IdDetAprob||'-'||:NEW.Cod_Pago, 'MODIFICA DETALLE_APROBACION', SUBSTR(cTexto,1,1000), cUsuario);
END;
...
ALTER TRIGGER "SICAS_OC"."TRG_ACTUALIZA_DET_APROBACION" ENABLE
