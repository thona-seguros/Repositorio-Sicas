CREATE OR REPLACE TRIGGER SICAS_OC.TRG_PERSONA_NATURAL_JURIDICA
  AFTER UPDATE OR DELETE OR INSERT ON SICAS_OC.PERSONA_NATURAL_JURIDICA
  FOR EACH ROW 
...
BEGIN
  /*TRIGGER PARA MONITOREO DE LA TABLA PERSONA_NATURAL_JURIDICA
    ...
    DESCRIPCIÓN: SE ACTUALIZA EL NUM TRIBUTARIO CADA VEZ QUE SE ACTUALIZA ALGUNO DE LOS DATOS CLAVE
  */
  ...
  IF UPDATING THEN 
    ...
    IF (NVL(:NEW.NUM_TRIBUTARIO,-20000) <> NVL(:OLD.NUM_TRIBUTARIO,-20000)) THEN
      ...
      IF (:NEW.TIPO_ID_TRIBUTARIA = 'RFC' OR :NEW.TIPO_DOC_IDENTIFICACION = 'RFC') THEN
        --ACTUALIZAR EL NUM_TRIBUTARIO SI EL TIPO PERSONA ES FISICO Y TIPO_DOC_IDENTIFICACION ES RFC
        cNum_Trib := SICAS_OC.OC_GENERARFC.GENERAPRINCIPAL(:OLD.NOMBRE,:OLD.APELLIDO_PATERNO,:OLD.APELLIDO_MATERNO,:OLD.FECNACIMIENTO);
      END IF;
    END IF;
