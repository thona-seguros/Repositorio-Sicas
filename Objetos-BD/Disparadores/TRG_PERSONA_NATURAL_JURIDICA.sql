...
BEGIN
  ...
  */
  ...
  IF INSERTING THEN 
      vl_Texto := TO_CHAR(:NEW.TIPO_DOC_IDENTIFICACION)||' - '||TO_CHAR(:NEW.NUM_DOC_IDENTIFICACION)||' - '||TO_CHAR(:NEW.NOMBRE)||' - '||TO_CHAR(:NEW.APELLIDO)||' - '||TO_CHAR(:NEW.APECASADA)||' - '||TO_CHAR(:NEW.SEXO)||' - '||
                  TO_CHAR(:NEW.ESTADOCIVIL)||' - '||TO_CHAR(:NEW.FECNACIMIENTO)||' - '||TO_CHAR(:NEW.DIRECRES)||' - '||TO_CHAR(:NEW.CODPAISRES)||' - '||TO_CHAR(:NEW.CODPROVRES)||' - '||TO_CHAR(:NEW.CODDISTRES)||' - '||
                  TO_CHAR(:NEW.CODCORRRES)||' - '||TO_CHAR(:NEW.ZIPRES)||' - '||TO_CHAR(:NEW.TELRES)||' - '||TO_CHAR(:NEW.EMPRESATRAB)||' - '||TO_CHAR(:NEW.DIRECOFI)||' - '||TO_CHAR(:NEW.CODPAISOFI)||' - '||
                  TO_CHAR(:NEW.CODPROVOFI)||' - '||TO_CHAR(:NEW.CODDISTOFI)||' - '||TO_CHAR(:NEW.CODCORROFI)||' - '||TO_CHAR(:NEW.ZIPOFI)||' - '||TO_CHAR(:NEW.TELOFI)||' - '||TO_CHAR(:NEW.FAXOFI)||' - '||
                  TO_CHAR(:NEW.EMAIL)||' - '||TO_CHAR(:NEW.FECSTS)||' - '||TO_CHAR(:NEW.FECINGRESO)||' - '||TO_CHAR(:NEW.CEDULA)||' - '||TO_CHAR(:NEW.TIPO_ID_TRIBUTARIA)||' - '||TO_CHAR(:NEW.NUM_TRIBUTARIO)||' - '||
                  TO_CHAR(:NEW.CODIGOZIP)||' - '||TO_CHAR(:NEW.CODPOSRES)||' - '||TO_CHAR(:NEW.CODASENRES)||' - '||TO_CHAR(:NEW.CODPOSOFI)||' - '||TO_CHAR(:NEW.CODASENOFI)||' - '||TO_CHAR(:NEW.TIPO_PERSONA)||' - '||
                  TO_CHAR(:NEW.TIP_DOCIDE_MATRIZ)||' - '||TO_CHAR(:NEW.NUM_DOCIDE_MATRIZ)||' - '||TO_CHAR(:NEW.TELMOVIL)||' - '||TO_CHAR(:NEW.APELLIDO_MATERNO)||' - '||TO_CHAR(:NEW.APELLIDO_PATERNO)||' - '||TO_CHAR(:NEW.CODCOLRES)||' - '||
                  TO_CHAR(:NEW.LADATELRES)||' - '||TO_CHAR(:NEW.LADATELOFI)||' - '||TO_CHAR(:NEW.LADAFAXOFI)||' - '||TO_CHAR(:NEW.LADATELMOVIL)||' - '||TO_CHAR(:NEW.CODACTIVIDAD)||' - '||TO_CHAR(:NEW.NUMINTERIOR)||' - '||
                  TO_CHAR(:NEW.NUMEXTERIOR)||' - '||TO_CHAR(:NEW.AUXCONTABLE)||' - '||TO_CHAR(:NEW.NOMBRECOMERCIAL)||' - '||TO_CHAR(:NEW.CURP)||' - '||TO_CHAR(:NEW.CODLISTAREF)||' - '||TO_CHAR(:NEW.FECLISTAREF)||' - '||
                  TO_CHAR(:NEW.NACIONALIDAD)||' - '||TO_CHAR(:NEW.IDCLIENTEUNICO)||' - '||TO_CHAR(:NEW.IDREGFISSAT)||' - '||TO_CHAR(:NEW.RAZONSOCIALFACT)||' - '||TO_CHAR(:NEW.FECMOVTOSICAS)||' - '||TO_CHAR(:NEW.FECAPLICASAT);

      vl_Primary := 'TIPO_DOC_IDENTIFICACION: '||:NEW.TIPO_DOC_IDENTIFICACION||' NUM_DOC_IDENTIFICACION: '||:NEW.NUM_DOC_IDENTIFICACION;
      OC_LOG_CONF_FACT_ELECT.INSERTA(1, 'PERSONA_NATURAL_JURIDICA', 'SE AGREGA NUEVO REGISTRO', vl_Texto, 'NUEVO REGISTRO', 'INSERT',vl_Primary);
  END IF;
