...
BEGIN
  ...
  */
  IF DELETING THEN 
      vl_Texto := TO_CHAR(:OLD.CODEMPRESA)||' - '||TO_CHAR(:OLD.CODCIA)||' - '||TO_CHAR(:OLD.IDTIPOSEG)||' - '||TO_CHAR(:OLD.PLANCOB)||' - '||TO_CHAR(:OLD.ID_AÑO)||' - '||TO_CHAR(:OLD.CODNIVEL)||' - '||TO_CHAR(:OLD.PORCCOMISION)
                  ||' - '||TO_CHAR(:OLD.ORIGEN)||' - '||TO_CHAR(:OLD.ST_COMISION)||' - '||TO_CHAR(:OLD.ID_USUARIO)||' - '||TO_CHAR(:OLD.FE_ESTATUS);
      vl_Primary := 'CODEMPRESA: '||:OLD.CODEMPRESA||' CODCIA: '||:OLD.CODCIA||' IDTIPOSEG: '||:OLD.IDTIPOSEG||' PLANCOB: '||:OLD.PLANCOB||' ID_AÑO: '||:OLD.ID_AÑO||' CODNIVEL: '||:OLD.CODNIVEL||' ST_COMISION: '||:OLD.ST_COMISION;
  ...
  IF INSERTING THEN 
      vl_Texto := TO_CHAR(:NEW.CODEMPRESA)||' - '||TO_CHAR(:NEW.CODCIA)||' - '||TO_CHAR(:NEW.IDTIPOSEG)||' - '||TO_CHAR(:NEW.PLANCOB)||' - '||TO_CHAR(:NEW.ID_AÑO)||' - '||TO_CHAR(:NEW.CODNIVEL)||' - '||TO_CHAR(:NEW.PORCCOMISION)
                  ||' - '||TO_CHAR(:NEW.ORIGEN)||' - '||TO_CHAR(:NEW.ST_COMISION)||' - '||TO_CHAR(:NEW.ID_USUARIO)||' - '||TO_CHAR(:NEW.FE_ESTATUS);
      vl_Primary := 'CODEMPRESA: '||:NEW.CODEMPRESA||' CODCIA: '||:NEW.CODCIA||' IDTIPOSEG: '||:NEW.IDTIPOSEG||' PLANCOB: '||:NEW.PLANCOB||' ID_AÑO: '||:NEW.ID_AÑO||' CODNIVEL: '||:NEW.CODNIVEL||' ST_COMISION: '||:NEW.ST_COMISION;
  ...
  IF UPDATING THEN 
    ...
    IF (:NEW.ID_AÑO <> :OLD.ID_AÑO) THEN
      vl_Primary := 'CODEMPRESA: '||:NEW.CODEMPRESA||' CODCIA: '||:NEW.CODCIA||' IDTIPOSEG: '||:NEW.IDTIPOSEG||' PLANCOB: '||:NEW.PLANCOB||' ID_AÑO: '||:NEW.ID_AÑO||' CODNIVEL: '||:NEW.CODNIVEL||' ST_COMISION: '||:NEW.ST_COMISION;
      OC_LOG_CONF_FACT_ELECT.INSERTA(:OLD.CODCIA, 'COMISION_PLACOB_LARPLA', 'ID_AÑO', TO_CHAR(:OLD.ID_AÑO), TO_CHAR(:NEW.ID_AÑO), 'UPDATE',vl_Primary);
    END IF;
