    ...
    IF INSERTING THEN 
        BEGIN
            ...
            VALUES (5,1,SICAS_OC.SEQ_PLD_OBSERVACIONES.NEXTVAL,:NEW.TIPO_PERSONA_CNSF,
                :NEW.LISTA_ORIGEN,:NEW.NOMBRE_1,NVL(:NEW.OBSERVACIONES,'Cambio de estatus desde SICAS por carga de Layout.'),SYSDATE,cUsuario);
                EXCEPTION
                    WHEN OTHERS THEN
                        vl_ErrorN := SQLCODE;
                ...
                BEGIN
                    IF (:OLD.OBSERVACIONES <> :NEW.OBSERVACIONES) AND (:NEW.OBSERVACIONES IS NOT NULL) THEN
                        ...
                        VALUES (5,1,SICAS_OC.SEQ_PLD_OBSERVACIONES.NEXTVAL,:OLD.TIPO_PERSONA_CNSF,
                            :OLD.LISTA_ORIGEN,:OLD.NOMBRE_1,NVL(:NEW.OBSERVACIONES,'Cambio de estatus desde SICAS por carga de Layout.'),SYSDATE,cUsuario);
                    END IF;
                EXCEPTION
                    WHEN OTHERS THEN
                        vl_ErrorN := SQLCODE;
